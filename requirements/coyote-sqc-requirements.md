...
statistics are generated by the following procedure, repeated `B = 100 000`
...
given the sorted bootstrap distribution of `B = 100 000` values
`b_{(1)} ≤ … ≤ b_{(B)}`:
...
```
LCL_j = b_{(r)}       where r = ⌊α_B / 2 · B⌋ = ⌊0.00027 · 100 000⌋ = 27
UCL_j = b_{(B − r + 1)} = b_{(99 974)}
CL_j  = b_{(50 000)}     (median of the bootstrap distribution)
```
...
A fixed random seed of `54 321` shall be used for all bootstrap computation
