...
| Incremental-markup qualification | REQ-CORE-047..049; default-off compatibility, enabled-mode format selection, chunk-boundary invariance, immediate per-delta rendering without timer batching, malformed-input fallback, Markdown replay compatibility, table/display-math completion boundaries, mixed native ordering, repeated native blocks, empty provisional-view cleanup, and malformed MathML-opening fallback | Restricted text/table/math slice implemented; focused, display-backed, and full-suite coverage passing |
