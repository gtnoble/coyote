...
package body Coyote_App is
   ...
   protected body Subagent_Model_Override is
      ...
      function Current return String is (To_String (P_Spec));
   ...
   protected body App_State is
      ...
      function Session_Id return String is (To_String (P_Session_Id));
      function Current_Model return String is (To_String (P_Model));
      function Current_Thinking return String is (To_String (P_Thinking));
      function Current_Sandbox return String is (To_String (P_Sandbox));
      function Source_Directory return String is
        (To_String (P_Source_Directory));
      function Session_Start return String is (To_String (P_Session_Start));
      function Current_Tool_Call return Natural is (P_Tool_Call);
      ...
      function Prompt_Filter return String is (To_String (P_Prompt_Filter));
      ...
      entry Wait_Frontend_Ready when P_Frontend_Ready or else P_Shutdown is
      begin
         null;
   ...
   procedure Run_GUI (Opts : Options) is
      ...
      Win_Name : constant String :=
        "coyote"
        &
        (if Length (Opts.Name) > 0 then " : " & To_String (Opts.Name) else "");
         ...
         if not State.Shutdown_Requested then
            declare
               ...
               begin
                  if E in LLM.Events.Message_Update_Event then
                     ...
                     begin
                        ...
                        Completion_Pending :=
                          not Agent_End.Was_Aborted and then not Opts.One_Shot
                     ...
                     begin
                        My_Frontend.Set_Stats_Summary
                          ((Model => To_Unbounded_String (State.Current_Model),
                            Session_Id     =>
               ...
               procedure Dispatch_Agent_Event
                 (E : LLM.Events.Agent_Event'Class) renames
                 Dispatch_Event;
               ...
               begin
                  ...
                  Render_Session_History
                    (UUID => UUID, Frontend => My_Frontend, State => State);
               ...
               procedure Run_Queued_Prompt
                 (Prompt : String; Is_Steer : Boolean)
               is
                  ...
                  when Ex : others =>
                     declare
                        Error_Text : constant String :=
                          (if
                             Length (Final_Error) > 0
                           then
                             To_String (Final_Error)
                           else Ada.Exceptions.Exception_Message (Ex));
                     begin
                        Append_Task_Warning ("prompt failed: " & Error_Text);
                        if Opts.One_Shot then
                           declare
            ...
            begin
               ...
               begin
                  if Parent_Sid'Length = 0 and then Inherited_Sid'Length > 0
                  then
                     Ada.Environment_Variables.Set
               ...
               begin
                  if Sess'Length > 0 then
                     Ada.Environment_Variables.Set ("COYOTE_SESSION_ID", Sess);
                  end if;
               end;
               ...
               if Ada.Strings.Unbounded.Length (Current_Thinking) > 0 then
                  ...
                  Ada.Environment_Variables.Set ("COYOTE_THINKING_LEVEL", "");
               end if;
               ...
               if Ada.Strings.Unbounded.Length (Current_Sandbox) > 0 then
                  ...
                  Ada.Environment_Variables.Set ("COYOTE_SANDBOX_PROFILE", "");
               end if;
                  ...
                  begin
                     case It.Kind is
                        ...
                        when Coyote_GUI.Prompt_Queue.User_Prompt =>
                           ...
                           begin
                              My_Frontend.Begin_Request
                                (Text => Prompt,
                                 Kind =>
                                   (if Is_Steer then Coyote_App.Frontend.Steer
                        ...
                        when Coyote_GUI.Prompt_Queue.Pause =>
                           if State.Is_Streaming and then not State.Is_Paused
                        ...
                        when Coyote_GUI.Prompt_Queue.New_Window =>
                           ...
                           begin
                              Args.Append
                                (Coyote_Utils.Active_Executable_Path);
                        ...
                        when Coyote_GUI.Prompt_Queue.New_Session =>
                           begin
                              ...
                              declare
                                 Settings_Value :
                                   constant LLM.Settings.Settings :=
                                   LLM.Settings.Load_Settings;
                              begin
                                 Ada.Environment_Variables.Set
                                   ("COYOTE_THINKING_LEVEL",
                                 ...
                                 Ada.Environment_Variables.Set
                                   ("COYOTE_SANDBOX_PROFILE",
                                    To_String
                                      (Settings_Value.Default_Sandbox));
                                 LLM.Agent.Create
                                   (S        => Agent_Session,
                                    Agent    => To_String (Opts.Agent),
                                    No_Tools => Opts.No_Tools);
                              ...
                              My_Frontend.Append_Notice
                                (Coyote_App.Frontend.Info,
                                 (if
                                    Spec'Length > 0
                                  then
                                    "Subagent model override set to " & Spec
                        ...
                        when Coyote_GUI.Prompt_Queue.Set_Thinking =>
                           begin
                              ...
                              LLM.Agent.Set_Thinking
                                (S => Agent_Session, Level => It.Level);
                              State.Set_Thinking
                        ...
                        when Coyote_GUI.Prompt_Queue.Set_Sandbox =>
                           begin
                              ...
                              State.Set_Sandbox (To_String (Current_Sandbox));
                              Ada.Environment_Variables.Set
                                ("COYOTE_SANDBOX_PROFILE",
                        ...
                        when Coyote_GUI.Prompt_Queue.Set_Preferences =>
                           begin
                              LLM.Settings.Save_Preferences
                                (Provider                  =>
                                   To_String (It.Preferences.Provider),
                                 Model_Id                  =>
                                   To_String (It.Preferences.Model_Id),
                                 Think_Level               =>
                                   Ada.Characters.Handling.To_Lower
                                     (LLM.Providers.Thinking_Level'Image
                                        (It.Preferences.Thinking)),
                                 Sandbox => To_String (It.Preferences.Sandbox),
                                 Subagent_Provider         =>
                                   To_String
                                     (It.Preferences.Subagent_Provider),
                                 Subagent_Model            =>
                                   To_String (It.Preferences.Subagent_Model),
                                 Max_Recursion_Depth       =>
               ...
               when Ex : others =>
                  ...
                  My_Frontend.Append_Notice
                    (Coyote_App.Frontend.Error,
                     "Agent task: " & Ada.Exceptions.Exception_Message (Ex));
                  Initiate_Shutdown;
            end;
         end if;
   ...
   begin
      ...
      Coyote_App.Frontend.GUI.Create
        (F                     => My_Frontend,
         ...
         Notifications_Allowed => not Opts.One_Shot and then not Opts.Subagent,
         Notifications_Enabled => Startup_Settings.Completion_Notifications);
      ...
      if Opts.One_Shot then
         ...
         begin
            if Json'Length > 0 then
               ...
               begin
                  Err.Set_Field
                    ("error", "subagent closed before producing output");
