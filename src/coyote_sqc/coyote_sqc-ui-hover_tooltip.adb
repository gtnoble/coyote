...
package body Coyote_SQC.UI.Hover_Tooltip is
   ...
   begin
      ...
      for I in 1 .. Positive (Coyote_SQC.App.State.Sessions.Length) loop
         if To_String (Coyote_SQC.App.State.Sessions.Element (I).Session_Id)
           = Session_Id
         then
            Sess_Found := True;
            S_Idx      := I;
      ...
      declare
         ...
         Ts        : constant String                               :=
           Ada.Calendar.Formatting.Image (Sess.Start_Time, Time_Zone => 0);
      ...
      begin
         ...
         begin
            if Coyote_SQC.Charts.Properties (Coyote_SQC.App.State.Active_Chart)
            ...
            then
               declare
                  Active : constant Coyote_SQC.Charts.Chart_Kind :=
                    Coyote_SQC.App.State.Active_Chart;
               begin
                  for Q of Coyote_SQC.App.State.Charts (Active).Quantile_Points
                  loop
         ...
         declare
            ...
            Latest_Ts_Str   : constant String  :=
              (if
                 Has_Latest
               then
                 Ada.Calendar.Formatting.Image (Latest_Time, Time_Zone => 0)
                   (1 .. 16)
               else "");
            C_Text          : constant String  := To_String (Latest_Text);
            C_Len : constant Natural := Natural'Min (C_Text'Length, 80);
            ...
            begin
               if QP_Found then
                  ...
                  Result :=
                    To_Unbounded_String
                      (NL & "n = " & Format_Number (Long_Long_Integer (QP.N))
                       & " turns" & NL & NL);
                  declare
                     ...
                     for Comp in Quantile_Index loop
                        declare
                           ...
                           UCL_Str : constant String                 :=
                             (if L.Has_UCL then Format_Float (L.UCL) else "-");
                           LCL_Str : constant String                 :=
                             (if L.Has_LCL then Format_Float (L.LCL) else "-");
                        begin
                           Result := Result &
                             (case Comp is
                                when Min_Q => "min",
                                ...
                                when Median_Q => "med",
                                when Q3 => "Q3",
                                when Max_Q => "max") & ":  " & Format_Float (V)
                             & " (UCL: " & UCL_Str & ", LCL: " & LCL_Str & ")";
                           if QP.OOC_Comps (Comp) then
                              Result :=
                                Result & "  " & Character'Val (16#E2#)
                                & Character'Val (16#86#)
                                & Character'Val (16#90#) & " out-of-control";
                           end if;
                           if Comp /= Max_Q then
                              Result := Result & NL;
                           end if;
                  ...
                  Result :=
                    To_Unbounded_String (NL & "CL: " & Format_Float (Pt.CL));
                  if Pt.Has_UCL then
                     Result := Result & "   UCL: " & Format_Float (Pt.UCL);
                  end if;
                  if Pt.Has_LCL then
                     Result := Result & "   LCL: " & Format_Float (Pt.LCL);
                  end if;
               end if;
               return To_String (Result);
            end Build_Stats_Line;
            Markup : constant String :=
              "<b>" & TS14 & "</b>" & "  " & Esc_Model & ASCII.LF & Esc_Src
              & ASCII.LF & ASCII.LF & "&quot;" & Esc_Prompt & "&quot;"
              & ASCII.LF & ASCII.LF & "Input: "
              & Format_Number (Long_Long_Integer (Sess.Total_Input_Tokens))
              & " tokens   Output: "
              & Format_Number (Long_Long_Integer (Sess.Total_Output_Tokens))
              & " tokens" & Build_Stats_Line
              &
              (if N_Comments > 0 then
                 ASCII.LF & "Comments: "
                 & Trim (N_Comments'Image, Ada.Strings.Left) & "  "
                 & Esc_Latest_Ts & ": &quot;" & Esc_Latest_Body & "&quot;"
               else "");
         begin
