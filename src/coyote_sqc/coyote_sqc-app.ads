...
package Coyote_SQC.App is
   ...
   type Chart_Point is record
      ...
      Hollow_Gray          : Boolean    :=
        False;  --  shown hollow gray (zero-thinking on thinking chart)
   ...
   package Chart_Point_Vectors is new Ada.Containers.Vectors
     (Index_Type => Positive, Element_Type => Chart_Point);
   ...
   package Quantile_Point_Vectors is new Ada.Containers.Vectors
     (Index_Type => Positive, Element_Type => Quantile_Point);
   ...
   type Chart_Data is record
      ...
      Transform_Active          : Coyote_SQC.Data_Model.Transform_Kind :=
        Coyote_SQC.Data_Model
          .None;  --  active transform (None = untransformed)
   ...
   type Chart_Data_Array is
     array (Coyote_SQC.Charts.Chart_Kind)
     of Chart_Data;
   ...
   type LF_Subgroup_Accessor is
     access function
       (M : Coyote_SQC.Data_Model.Session_Metrics_Record)
        return Coyote_SQC.Data_Model.Long_Float_Vectors.Vector;
