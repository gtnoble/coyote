...
package body Coyote_SQC.UI is
   ...
   Recent_Items   : array (1 .. Coyote_SQC.Config.Max_Recent)
     of Gtk.Menu_Item.Gtk_Menu_Item := (others => null);
   ...
   procedure On_Show_All
     (Item : access Gtk.Menu_Item.Gtk_Menu_Item_Record'Class);
   procedure On_Y_Fit (Item : access Gtk.Menu_Item.Gtk_Menu_Item_Record'Class);
   ...
   begin
      if Coyote_SQC.App.State /= null and then Coyote_SQC.App.State.Modified
      then
         declare
            ...
            Res : constant Coyote_SQC.UI.Dialogs.Dialog_Response :=
              Coyote_SQC.UI.Dialogs.Unsaved_Changes
                (Coyote_SQC.App.State.Main_Window,
                 To_String (Coyote_SQC.App.State.Workspace.Name));
         begin
            if Res = Coyote_SQC.UI.Dialogs.Response_Cancel then
               return;
            end if;
            ...
            begin
               ...
               Coyote_SQC.App.State.Workspace      :=
                 (Workspace_Id =>
                  ...
                  Name         =>
                    To_Unbounded_String
                      (if Name'Length > 0 then Name else "New Workspace"),
                  others       => <>);
      ...
      if Res = Gtk_Response_OK then
         ...
         begin
            ...
            begin
               ...
               Coyote_SQC.App.State.Workspace_Path :=
            ...
            Res : constant Dialog_Response :=
              Coyote_SQC.UI.Dialogs.Unsaved_Changes
                (Coyote_SQC.App.State.Main_Window,
                 To_String (Coyote_SQC.App.State.Workspace.Name));
         begin
            if Res = Response_Cancel then
               return;
            end if;
            if Res = Response_OK then
               On_Save (null);
            end if;
         end;
      end if;
   ...
   begin
      if Coyote_SQC.App.State = null
        or else Coyote_SQC.App.State.Recent_Menu = null
      then
         return;
      end if;
      ...
      for I in 1 .. Integer (Recent.Count) loop
         declare
            E    : constant Recent_Entry := Recent.Entries (I);
            Lbl  : constant String       :=
              To_String (E.Name) & "  (" & To_String (E.Path) & ")";
   ...
   procedure On_Reload (Item : access Gtk.Menu_Item.Gtk_Menu_Item_Record'Class)
   is
      pragma Unreferenced (Item);
   begin
      if Coyote_SQC.App.State /= null then
   ...
   begin
      ...
      if Coyote_SQC.App.State.Selection.Is_Empty then
         return;
      end if;
   ...
   begin
      ...
      begin
         Gtk.Menu_Item.Gtk_New (SASI, "Set Selection as Setup Interval");
   ...
   begin
      ...
      Win.Set_Default_Size (1_200, 700);
