...
package body LLM.Providers.Ollama is
   ...
   is
      ...
      Stop_Reason       : LLM.Types.Stop_Reason := LLM.Types.Unknown_Stop;

      procedure Process_Line
        (Line_Str : String;
         H        : not null access procedure (E : LLM.Events.Agent_Event'Class))
      is
         Parsed : constant GNATCOLL.JSON.Read_Result := GNATCOLL.JSON.Read (Line_Str);
      ...
      begin
         ...
         then
            ...
            if Root.Has_Field ("prompt_eval_count") then
               declare
                  N : constant Long_Integer := Root.Get ("prompt_eval_count").Get;
               begin
                  if N >= 0 then
                     Total_Usage.Input := Total_Usage.Input + Natural (N);
                  end if;
               end;
            end if;
            if Root.Has_Field ("eval_count") then
               declare
                  N : constant Long_Integer := Root.Get ("eval_count").Get;
               begin
                  if N >= 0 then
                     Total_Usage.Output := Total_Usage.Output + Natural (N);
                  end if;
               end;
            end if;
            return;
         ...
         if Root.Has_Field ("message")
           and then Root.Get ("message").Kind = GNATCOLL.JSON.JSON_Object_Type
         then
            declare
               Msg : constant GNATCOLL.JSON.JSON_Value := Root.Get ("message");
            begin
               --  Text content
               ...
               if Msg.Has_Field ("tool_calls")
                 and then Msg.Get ("tool_calls").Kind = GNATCOLL.JSON.JSON_Array_Type
               then
                  declare
                  ...
                  begin
                     if GNATCOLL.JSON.Length (TC_Array) > 0 then
                        ...
                        begin
                           if TC_Obj.Kind = GNATCOLL.JSON.JSON_Object_Type then
                              declare
                                 Func_Obj : constant GNATCOLL.JSON.JSON_Value :=
                                   Get_Object_Field (TC_Obj, "function");
                              begin
                                 if Func_Obj.Kind = GNATCOLL.JSON.JSON_Object_Type then
                                    if Func_Obj.Has_Field ("name")
                                      and then Func_Obj.Get ("name").Kind = GNATCOLL.JSON.JSON_String_Type
                                    then
                                       Tool_Name := To_Unbounded_String (Func_Obj.Get ("name").Get);
                                    end if;
                                    if Func_Obj.Has_Field ("arguments") then
                                       Arguments_Json := To_Unbounded_String (GNATCOLL.JSON.Write (Func_Obj.Get ("arguments")));
                                    end if;
                                 end if;
                              end;
                           end if;
         ...
         loop
            declare
               LF_Pos : constant Natural :=
                 Ada.Strings.Fixed.Index (To_String (Response_Body), ASCII.LF);
            begin
               if LF_Pos = 0 then
                  exit;
               end if;
               declare
                  Line : constant String :=
                    To_String (Response_Body) (1 .. LF_Pos - 1);
               begin
                  Process_Line (Line, Handler);
               end;
               --  Remove processed line and LF
               Response_Body := To_Unbounded_String
                 (To_String (Response_Body) (LF_Pos + 1 .. To_String (Response_Body)'Last));
            end;
         end loop;
      end On_Chunk;

      Request_Obj  : constant GNATCOLL.JSON.JSON_Value := GNATCOLL.JSON.Create_Object;
      Messages_Arr : GNATCOLL.JSON.JSON_Array := GNATCOLL.JSON.Empty_Array;
      Effective_Base_Url : String := To_String (P.Base_Url);
      Effective_Api_Key  : String := To_String (P.Api_Key);
   begin
      Handler.all (LLM.Events.Agent_Start_Event);
      ...
      if Effective_Base_Url'Length = 0 then
         ...
         declare
            ...
            Prov : constant GNATCOLL.JSON.JSON_Value :=
              Find_Provider_Config (Root, "ollama");
         begin
            if Prov.Kind = GNATCOLL.JSON.JSON_Object_Type then
               if Prov.Has_Field ("baseUrl")
                 and then Prov.Get ("baseUrl").Kind = GNATCOLL.JSON.JSON_String_Type
               then
                  Effective_Base_Url := Prov.Get ("baseUrl").Get;
               end if;
            end if;
            if Effective_Api_Key'Length = 0 then
               Effective_Api_Key := LLM.Settings.Resolve_Api_Key ("ollama");
            end if;
         end;
      end if;

      if Effective_Api_Key'Length > 0 then
         LLM.HTTP.Add_Header (Headers, "Authorization", "Bearer " & Effective_Api_Key);
      end if;
      ...
      for I in 1 .. Messages.Length loop
         declare
            M      : constant LLM.Types.Message := Messages.Element (I);
            Msg_Obj : constant GNATCOLL.JSON.JSON_Value := GNATCOLL.JSON.Create_Object;
         ...
         begin
            ...
            for J in 1 .. M.Content.Length loop
               declare
                  Block : constant LLM.Types.Content_Block := M.Content.Element (J);
               begin
                  case Block.Kind is
            ...
            Msg_Obj.Set_Field ("content", To_String (Content));
      ...
      Handler.all
        (LLM.Events.Message_End_Event'
          (Stop      => Stop_Reason,
           ...
           Cost_Dmil => 0));
      Handler.all (LLM.Events.Agent_End_Event);
         ...
         Handler.all
           (LLM.Events.Message_End_Event'
             (Stop      => LLM.Types.Error_Stop,
              Err_Msg   => To_Unbounded_String (Ada.Exceptions.Exception_Message (E)),
              Tok_Usage => (others => 0),
              Cost_Dmil => 0));
         Handler.all (LLM.Events.Agent_End_Event'(Was_Aborted => False));
         raise;
   end Send;
