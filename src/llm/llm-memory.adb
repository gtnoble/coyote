...
package body LLM.Memory is
   ...
   begin
      ...
      begin
            ...
            declare
               Next_LF : constant Natural :=
                 Ada.Strings.Fixed.Index
                   (Capped_Str (LF_Pos + 1 .. Capped_Str'Last), "" & ASCII.LF);
            begin
               exit when Next_LF = 0;
         ...
         if LF_Pos >= Capped_Str'First and then LF_Pos < Capped_Str'Last then
            Capped    :=
              To_Unbounded_String (Capped_Str (Capped_Str'First .. LF_Pos));
            Truncated := True;
         end if;
      end;
   ...
   function Load_Memory_Index (Cwd : String) return String is
      ...
      Global_Path  : constant String :=
        (if Agent_Dir'Length > 0 then Agent_Dir & "/memory/MEMORY.md" else "");
      Project_Path : constant String :=
        (if Cwd'Length > 0 then Cwd & "/.coyote/MEMORY.md" else "");
      Result       : Unbounded_String;
   begin
      if Global_Path'Length > 0 and then Ada.Directories.Exists (Global_Path)
        and then Ada.Directories.Kind (Global_Path)
          = Ada.Directories.Ordinary_File
      then
         declare
            Content : constant String := Load_Single_Memory_File (Global_Path);
         begin
            if Content'Length > 0 then
               Append
                 (Result,
                  "# User Memory (global)" & ASCII.LF & ASCII.LF & Content
                  & ASCII.LF);
            end if;
         end;
      end if;

      if Project_Path'Length > 0 and then Ada.Directories.Exists (Project_Path)
      ...
      then
         ...
         begin
            if Content'Length > 0 then
               Append
                 (Result,
                  "# Project Memory" & ASCII.LF & ASCII.LF & Content
                  & ASCII.LF);
            end if;
         end;
      end if;
   ...
   begin
      return
        "# Memory System" & ASCII.LF & ASCII.LF
        ...
        & " edge cases rather than blindly following the rule." & ASCII.LF
        & "- **When to save:** After every user correction or explicit"
        & " confirmation of a design/implementation choice." & ASCII.LF
        & "- **How to use:** Apply corrections as constraints; use the Why:"
        & " rationale to determine scope and exceptions." & ASCII.LF & ASCII.LF
        & "### project" & ASCII.LF
