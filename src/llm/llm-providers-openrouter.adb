...
package body LLM.Providers.OpenRouter is
   ...
   begin
      if Session_Id'Length > MAX_SESSION_ID_LENGTH then
         raise Constraint_Error
           with "OpenRouter session_id exceeds 256 characters";
      end if;
      ...
      return Result : Provider do
         ...
         Add_Header
           (Result, "HTTP-Referer", "https://github.com/gtnoble/coyote");
   ...
   begin
      ...
      if Ada.Strings.Unbounded.Length (P.Session_Id) > 0 then
         Request.Set_Field
           ("session_id", Ada.Strings.Unbounded.To_String (P.Session_Id));
      end if;
   ...
   begin
      ...
      if Api_Key'Length = 0 then
         raise LLM.HTTP.Curl_Error
           with "OpenRouter API key is not configured; set OPENROUTER_API_KEY or "
