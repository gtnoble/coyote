...
package body LLM.Providers.OpenAI_Completions is
   ...
   function To_Stop_Reason
     (Finish_Reason : String) return LLM.Types.Stop_Reason
   is
   begin
   ...
   begin
      return
        (Input       => Get_Natural_Field (Value, "prompt_tokens"),
         ...
         Cache_Read  =>
           (if
              Get_Natural_Field (Details, "cached_tokens") > 0
            then
   ...
   procedure Append_User_Message
     (Messages : in out GNATCOLL.JSON.JSON_Array; Msg : LLM.Types.Message)
   is
      Message : constant GNATCOLL.JSON.JSON_Value :=
        GNATCOLL.JSON.Create_Object;
   begin
   ...
   begin
      ...
      if Length (Media_Type) > 0 then
         ...
         begin
            ...
            User_Msg.Set_Field ("content", GNATCOLL.JSON.Create (Content));
   ...
   begin
      ...
      Append_System_Message (Msgs, System_Prompt, P.Inline_Cache_Hints);
               ...
               if GNATCOLL.JSON.Length (Raw_Tools) > 0 then
                  ...
                  begin
                     for I in 1 .. GNATCOLL.JSON.Length (Raw_Tools) loop
                        ...
                        begin
                           if I = GNATCOLL.JSON.Length (Raw_Tools)
   ...
   begin
      for I in 1 .. GNATCOLL.JSON.Length (Tool_Calls) loop
         declare
            ...
            Id_Fragment    : constant String := Get_String_Field (Item, "id");
            ...
            Delta_Fragment : constant String                   :=
              (if Args_Fragment'Length > 0 then Args_Fragment
               else Name_Fragment);
         begin
               ...
               if Id_Fragment'Length > 0 then
                  if Length (Tool_State.Tool_Call_Id) = 0 then
                     Tool_State.Tool_Call_Id :=
                       To_Unbounded_String (Id_Fragment);
                  else
   ...
   begin
      ...
      begin
         ...
         if Has_String_Field (Delta_Value, "reasoning")
           or else Has_String_Field (Delta_Value, "reasoning_content")
         then
            ...
            Emit_Update
              (Handler    => Handler,
               ...
               Delta_Text =>
                 Normalize_Thinking_Delta
                   (Get_String_Field (Delta_Value, "reasoning")
                    & Get_String_Field (Delta_Value, "reasoning_content")));
         end if;

         declare
            Content : constant String :=
              Get_String_Field (Delta_Value, "content");
         begin
   ...
   begin
      ...
      begin
         ...
         if Has_String_Field (Message, "reasoning")
           or else Has_String_Field (Message, "reasoning_content")
         then
            Emit_Update (Handler, LLM.Events.Thinking_Start);
            Emit_Update
              (Handler    => Handler,
               Kind       => LLM.Events.Thinking_Delta,
               Delta_Text =>
                 Get_String_Field (Message, "reasoning")
                 & Get_String_Field (Message, "reasoning_content"));
            State.Thinking_Started := True;
         end if;
