...
package Coyote_Cmark is
   ...
   function Parse_Document
     (Buffer  : Interfaces.C.char_array;
      ...
      return Node_Ptr with
     Import, Convention => C, External_Name => "cmark_shim_parse_document_gfm";

   --  Free a document node tree returned by Parse_Document.
   procedure Node_Free (Node : Node_Ptr) with
     Import, Convention => C, External_Name => "cmark_node_free";

   --  Return the node type constant for Node.
   function Node_Get_Type (Node : Node_Ptr) return Node_Type_Int with
     Import, Convention => C, External_Name => "cmark_node_get_type";

   --  Return the inclusive source line range occupied by Node.
   function Node_Get_Start_Line (Node : Node_Ptr) return Interfaces.C.int with
     Import, Convention => C, External_Name => "cmark_node_get_start_line";

   function Node_Get_End_Line (Node : Node_Ptr) return Interfaces.C.int with
     Import, Convention => C, External_Name => "cmark_node_get_end_line";

   --  Return the heading level (1..6) for NODE_HEADING nodes.
   function Node_Get_Heading_Level
     (Node : Node_Ptr) return Interfaces.C.int with
     Import, Convention => C, External_Name => "cmark_node_get_heading_level";

   --  Return the list type for NODE_LIST nodes.
   function Node_Get_List_Type (Node : Node_Ptr) return List_Type_Int with
     Import, Convention => C, External_Name => "cmark_node_get_list_type";

   --  Return the starting ordinal for NODE_LIST ordered lists.
   function Node_Get_List_Start (Node : Node_Ptr) return Interfaces.C.int with
     Import, Convention => C, External_Name => "cmark_node_get_list_start";
   ...
   function Iter_New (Root : Node_Ptr) return Iter_Ptr with
     Import, Convention => C, External_Name => "cmark_iter_new";

   --  Advance the iterator; returns the event type for the current node.
   function Iter_Next (Iter : Iter_Ptr) return Event_Type_Int with
     Import, Convention => C, External_Name => "cmark_iter_next";

   --  Return the node associated with the most recent Iter_Next call.
   function Iter_Get_Node (Iter : Iter_Ptr) return Node_Ptr with
     Import, Convention => C, External_Name => "cmark_iter_get_node";

   --  Free an iterator created by Iter_New.
   procedure Iter_Free (Iter : Iter_Ptr) with
     Import, Convention => C, External_Name => "cmark_iter_free";
   ...
   function Table_Column_Count (Node : Node_Ptr) return Interfaces.C.int with
     Import, Convention => C, External_Name => "cmark_shim_table_column_count";
   ...
   function Table_Column_Alignment
     (Node : Node_Ptr; Column : Interfaces.C.int) return Interfaces.C.int with
     Import,
     Convention    => C,
     External_Name => "cmark_shim_table_column_alignment";

   --  Return 1 if Node is a table_row that is the header row, else 0.
   function Table_Row_Is_Header (Node : Node_Ptr) return Interfaces.C.int with
     Import,
     Convention    => C,
     External_Name => "cmark_shim_table_row_is_header";

   --  Return the first child of Node, or System.Null_Address if none.
   function Node_First_Child (Node : Node_Ptr) return Node_Ptr with
     Import, Convention => C, External_Name => "cmark_node_first_child";

   --  Return the next sibling of Node, or System.Null_Address if none.
   function Node_Next (Node : Node_Ptr) return Node_Ptr with
     Import, Convention => C, External_Name => "cmark_node_next";
