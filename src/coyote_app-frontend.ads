...
package Coyote_App.Frontend is
   ...
   type Control_Command is record
      ...
      Abort_Message   : Ada.Strings.Unbounded.Unbounded_String;
   end record;

   function Has_Control_Channel (F : Instance) return Boolean is (False);
   function Read_Control
     (F : in out Instance; Command : out Control_Command) return Boolean is
     (False);
   ...
   procedure Set_Mode (F : in out Instance; Mode : Run_Mode) is abstract;
   ...
   procedure Append_Thinking (F : in out Instance; Text : String) is abstract;
   ...
   procedure Set_Tool_Status
     (F : in out Instance; Tool_Id : String; Status : Tool_Status) is abstract;
   ...
   procedure Complete_Request
     (F : in out Instance; Status : Completion_Status) is null;
   ...
   procedure Append_Notice
     (F : in out Instance; Kind : Notice_Kind; Text : String) is abstract;

   --  Show a named block of content outside the main conversation view.
   procedure Show_Detail
     (F : in out Instance; Title : String; Content : String) is abstract;

   --  Return the next prompt, or "" when the frontend is closed.
   function Read_Prompt (F : in out Instance) return String is abstract;
