...
package LLM_Agent_Tests is
   ...
   procedure Test_Abort_Batched_Tools_Keep_History_Valid (T : in out Test);
   procedure Test_Abort_During_Shell_With_Timeout (T : in out Test);
   procedure Test_Session_File_Written_Only_After_Turn_End (T : in out Test);
   ...
   procedure Test_Multi_Turn_Same_Session_Carries_History (T : in out Test);
   ...
   procedure Test_Unknown_Tool_Becomes_Error_And_Agent_Continues
     (T : in out Test);
   procedure Test_Auto_Retry_On_HTTP_500_Then_Success (T : in out Test);
   ...
   procedure Test_Compatible_History_Filters_Foreign_Thinking
     (T : in out Test);
   procedure Test_Non_Retryable_Error_Rolls_Back_Prompt (T : in out Test);
   procedure Test_Retry_Exhaustion_Rolls_Back_Prompt (T : in out Test);
   procedure Test_Is_Context_Overflow_Error_Detects_Known_Phrases
     (T : in out Test);
   procedure Test_Overflow_Triggers_Compact_And_Retry (T : in out Test);
   procedure Test_Overflow_Recovery_Not_Attempted_Twice (T : in out Test);
   procedure Test_Overflow_Will_Retry_Event_Emitted (T : in out Test);
   procedure Test_Compact_Produces_Summary_Message (T : in out Test);
   procedure Test_Compact_Emits_Start_And_End_Events (T : in out Test);
   procedure Test_Compact_Short_History_Aborts (T : in out Test);
   procedure Test_Compact_Persists_Entry (T : in out Test);
   procedure Test_Auto_Compact_Fires_At_Threshold (T : in out Test);
   ...
   procedure Test_Auto_Compact_Session_Persisted_After_Threshold
     (T : in out Test);
   procedure Test_Set_Compact_Settings_Disabled (T : in out Test);
   procedure Test_Compact_Then_Resume (T : in out Test);
   procedure Test_Compact_Live_Summarises_Conversation (T : in out Test);
   procedure Test_Tool_Result_Has_Stats_Footer (T : in out Test);
   procedure Test_Stats_Footer_Only_On_Last_Tool_In_Batch (T : in out Test);
   ...
   --  A persistent sandbox default applies when no inherited profile exists.
   procedure Test_Sandbox_Default_From_Settings (T : in out Test);
