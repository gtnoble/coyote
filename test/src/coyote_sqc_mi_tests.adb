...
package body Coyote_SQC_MI_Tests is
      ...
      for I in 1 .. Natural (Result.Length) loop
         Assert
           (Result.Element (I) > 0.0,
            ...
            & "] must be > 0; got " & Long_Float'Image (Result.Element (I)));
      end loop;
   ...
   begin
      ...
      for V of Result_Same loop
         Sum_Same := Sum_Same + V;
      end loop;
      for V of Result_Diff loop
         Sum_Diff := Sum_Diff + V;
      end loop;
      ...
      Assert
        (Sum_Same >= Sum_Diff,
         ...
         & Long_Float'Image (Sum_Same) & " Diff="
         & Long_Float'Image (Sum_Diff));
   ...
   begin
      ...
      Assert
        (Result.Element (2) <= Long_Float'Last,
         "One-side absent: arg key 'a' MI_k must be finite");
      Assert
        (Result.Element (3) <= Long_Float'Last,
   ...
   begin
      ...
      Assert
        (Result.Is_Empty, "Both sides empty: should produce zero MI values");
   ...
   begin
      ...
      Assert
        (M.N_Consecutive_Tool_MI_Pairs = 0,
         "Single tool call: N_MI_Pairs must be 0");
      Assert
        (M.Per_Consecutive_Tool_MI.Is_Empty,
   ...
   begin
      ...
      Assert
        (abs (M.Total_Tool_Call_MI - Manual) <= Tol,
         "Total_Tool_Call_MI must equal sum of Per_Consecutive_Tool_MI; "
         & "Total=" & Long_Float'Image (M.Total_Tool_Call_MI) & " Manual="
   ...
   package SQC_MI_Caller is new AUnit.Test_Caller (Coyote_SQC_MI_Tests.Test);
      ...
      Result.Add_Test
        (SQC_MI_Caller.Create
           ("SQC MI: estimate excludes no-pairs",
            Coyote_SQC_MI_Tests.Test_Estimate_MI_Sum_Excludes_No_Pairs'
              Access));
