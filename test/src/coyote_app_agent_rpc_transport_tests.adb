...
package body Coyote_App_Agent_RPC_Transport_Tests is
   ...
   begin
      ...
      Assert
        (Receive_Frame (Right, Received, Status, Error),
         "peer must receive a complete handshake frame");
      Assert (Status = Frame_Received, "handshake status must be received");
      Assert
        (Received.Kind = Handshake, "received frame kind must be handshake");
      Assert
        (To_String (Received.Agent_Id) = "worker",
   ...
   begin
      ...
      Close (Left);
      Assert
        (not Receive_Frame (Right, Received, Status, Error),
         "peer close must return false");
      Assert
        (Status = Peer_Closed, "peer close must have the peer-closed status");
      Close (Right);
   ...
   begin
      ...
      Assert
        (Ada.Calendar.Clock - Started < 0.5,
   ...
   procedure Test_Unix_Listener_Times_Out_When_Idle (T : in out Test) is
      ...
      Client   : Channel;
      Path : constant String := Test_Endpoint ("coyote-rpc-transport-timeout");
   ...
   procedure Test_Unix_Listener_Accepts_Client (T : in out Test) is
      ...
      Path     : constant String :=
        Test_Endpoint ("coyote-rpc-transport-listener");
      Value    : Frame;
      Status   : Receive_Status;
      Error    : Unbounded_String;
   ...
   begin
      ...
      Assert
        (Receive_Frame (Server, Value, Status, Error),
         "accepted channel must receive client handshake");
      Assert
        (Value.Kind = Handshake, "accepted channel frame must be a handshake");
      Close (Client);
      Close (Server);
      Close (Listener);
   end Test_Unix_Listener_Accepts_Client;

   package Agent_RPC_Transport_Caller is new AUnit.Test_Caller
     (Coyote_App_Agent_RPC_Transport_Tests.Test);
   ...
   begin
      Result.Add_Test
        (Agent_RPC_Transport_Caller.Create
