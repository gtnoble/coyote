...
package body LLM_SSE_Tests is
   ...
   Full_Event_Fixture : constant String :=
     "event: tool_call" & ASCII.LF & "data: {""name"":""read""," & ASCII.LF
     & "data: ""path"":""src/main.adb""}" & ASCII.LF & ASCII.LF;

   Anthropic_Fixture : constant String :=
     "event: message_start" & ASCII.LF
     & "data: {""type"":""message_start"",""message"":{""id"":""msg_1""}}"
     & ASCII.LF & ASCII.LF & "event: content_block_delta" & ASCII.LF
     & "data: {""type"":""content_block_delta"",""index"":0,"
     & """delta"":{""type"":""thinking_delta"",""thinking"":""plan""}}"
     & ASCII.LF & ASCII.LF;

   OpenAI_Fixture : constant String :=
     "data: {""choices"":[{""delta"":{""content"":""Hello""}}]}" & ASCII.LF
     & ASCII.LF & "data: {""choices"":[{""delta"":{""content"":"" world""}}]}"
     & ASCII.LF & ASCII.LF & "data: [DONE]" & ASCII.LF & ASCII.LF;

   Ping_Fixture : constant String :=
     "event: ping" & ASCII.LF & "data: {""type"":""ping""}" & ASCII.LF
     & ASCII.LF & "data: {""choices"":[{""delta"":{""content"":""ok""}}]}"
     & ASCII.LF & ASCII.LF;

   CRLF_Ping_Fixture : constant String :=
     "data: hello" & ASCII.CR & ASCII.LF & "event: ping" & ASCII.CR & ASCII.LF
     & ASCII.CR & ASCII.LF & "data: world" & ASCII.CR & ASCII.LF & ASCII.CR
     & ASCII.LF;
   ...
   begin
      ...
      Assert
        (To_String (Event_Name) = Expected_Name, "Unexpected SSE event name");
   ...
   begin
      ...
      Assert_Event
        (P,
         ...
         Expected_Data =>
           "{""name"":""read""," & ASCII.LF & """path"":""src/main.adb""}");
   ...
   procedure Test_Multi_Chunk_Event (T : in out Test) is
      ...
      Data       : Unbounded_String;
   begin
      LLM.SSE.Feed (P, "event: content_block_delta" & ASCII.LF & "data: {""");
      Assert
        (not LLM.SSE.Next_Event (P, Event_Name, Data),
         "Partial event should not be returned yet");

      LLM.SSE.Feed
        (P, "type"":""content_block_delta"",""delta"":{""text"":""hel");
      Assert
        (not LLM.SSE.Next_Event (P, Event_Name, Data),
   ...
   begin
      ...
      Assert_Event
        (P,
         Expected_Name => "",
         Expected_Data => "{""choices"":[{""delta"":{""content"":""ok""}}]}");
   ...
   package LLM_SSE_Caller is new AUnit.Test_Caller (LLM_SSE_Tests.Test);
