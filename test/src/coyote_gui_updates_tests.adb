...
package body Coyote_GUI_Updates_Tests is
   ...
   begin
      ...
      Queue.Enqueue (Sample_Update, Wake);
      Assert
        (not Wake, "pending enqueue must not request a second idle source");
   ...
   procedure Test_Idle_Done_Keeps_Source_For_Pending_Work (T : in out Test) is
      pragma Unreferenced (T);
      Queue       : Coyote_GUI.Updates.Queue;
      U           : Coyote_GUI.Update;
      Got         : Boolean;
   ...
   begin
      ...
      Assert
        (Keep_Active, "idle source must remain active while work is pending");
      Queue.Enqueue (Sample_Update, Wake);
      Assert
        (not Wake, "pending work must not register a competing idle source");
   ...
   begin
      ...
      Assert
        (To_String (Output.Runtime_Agent_Id) = "agent-7",
         "runtime agent identity must survive update transport");
      Assert
        (To_String (Output.Text) = "child output",
   ...
   begin
      ...
      Assert
        (To_String (Output.Text2) = To_String (Input.Text2),
      ...
      Result.Add_Test
        (Coyote_GUI_Updates_Caller.Create
           ("Coyote.GUI.Updates first enqueue wakes exactly once",
            Coyote_GUI_Updates_Tests.Test_First_Enqueue_Wakes_Exactly_Once'
              Access));
      Result.Add_Test
        (Coyote_GUI_Updates_Caller.Create
           ("Coyote.GUI.Updates pending enqueue does not duplicate wakeup",
            Coyote_GUI_Updates_Tests
              .Test_Pending_Enqueue_Does_Not_Duplicate_Wakeup'
              Access));
      Result.Add_Test
        (Coyote_GUI_Updates_Caller.Create
           ("Coyote.GUI.Updates idle completion keeps source for pending work",
            Coyote_GUI_Updates_Tests
              .Test_Idle_Done_Keeps_Source_For_Pending_Work'
              Access));
      Result.Add_Test
        (Coyote_GUI_Updates_Caller.Create
           ("Coyote.GUI.Updates idle completion clears source when empty",
            Coyote_GUI_Updates_Tests.Test_Idle_Done_Clears_Source_When_Empty'
              Access));
      Result.Add_Test
        (Coyote_GUI_Updates_Caller.Create
           ("Coyote.GUI.Updates enqueue rearms after idle completion",
            Coyote_GUI_Updates_Tests.Test_Enqueue_Rearms_After_Idle_Done'
              Access));
      Result.Add_Test
        (Coyote_GUI_Updates_Caller.Create
      ...
      Result.Add_Test
        (Coyote_GUI_Updates_Caller.Create
           ("Coyote.GUI.Updates preserves runtime agent identity",
            Coyote_GUI_Updates_Tests.Test_Runtime_Agent_Id_Round_Trips'
              Access));
      Result.Add_Test
        (Coyote_GUI_Updates_Caller.Create
