...
package body LLM_OpenCode_Go_Catalogue_Tests is
   ...
   begin
      ...
      Assert
        (LLM.Providers.OpenCode_Go.Catalogue.Wire_Format_For ("gpt-5.6-luna")
         = LLM.Providers.OpenCode_Go.Catalogue.OpenAI_Responses_Wire,
   ...
   procedure Test_OpenAI_Request_Omits_Cache_Hints (T : in out Test) is
      ...
      Base_Url       : constant String := "http://127.0.0.1:18810";
      ...
      Captured       : GNATCOLL.JSON.JSON_Value := GNATCOLL.JSON.JSON_Null;
      ...
      begin
         if Base_Was_Set then
            ...
            Ada.Environment_Variables.Clear ("COYOTE_OPENCODE_GO_BASE_URL");
         end if;
      ...
      begin
         ...
         Append
           (Res.Body_Data,
            ...
            & ASCII.LF & ASCII.LF & "data: [DONE]" & ASCII.LF & ASCII.LF);
      end Handle_Request;
   ...
   begin
      Ada.Environment_Variables.Set ("COYOTE_OPENCODE_GO_BASE_URL", Base_Url);
      ...
      begin
         ...
         Assert
           (not GNATCOLL.JSON.Get (Tools_Json, 1).Has_Field ("cache_control"),
      ...
      begin
         ...
         Append
           (Res.Body_Data,
            ...
            & """response"":{ ""status"":""in_progress""}}" & ASCII.LF
            & ASCII.LF & "event: response.output_item.added" & ASCII.LF
            & "data: {""type"":""response.output_item.added"","
            & """item"":{ ""type"":""message""," & """id"":""msg_test""}}"
            & ASCII.LF & ASCII.LF & "event: response.output_text.delta"
            & ASCII.LF & "data: {""type"":""response.output_text.delta"","
            & """item_id"":""msg_test""," & """delta"":""OK""}" & ASCII.LF
            & ASCII.LF & "event: response.output_text.done" & ASCII.LF
            & "data: {""type"":""response.output_text.done""}" & ASCII.LF
            & ASCII.LF & "event: response.completed" & ASCII.LF
   ...
   procedure Test_Static_Metadata_Known_Model (T : in out Test) is
      pragma Unreferenced (T);
      Cat_Models :
        LLM.Providers.OpenCode_Go.Catalogue.Catalogue_Vectors.Vector;
   begin
   ...
   package LLM_OpenCode_Go_Catalogue_Caller is new AUnit.Test_Caller
     (LLM_OpenCode_Go_Catalogue_Tests.Test);
