...
package body LLM_Anthropic_Messages_Tests is
   ...
   function Event_Record
     (Event_Name : String; Data : GNATCOLL.JSON.JSON_Value) return String
   is
   begin
      return
        "event: " & Event_Name & ASCII.LF & "data: "
   ...
   begin
      ...
      return
        Event_Record ("message_start", Message_Start)
   ...
   Anthropic_SSE_Payload : constant String :=
     "event: message_start" & ASCII.LF & "data: {""type"":""message_start"","
     & """message"":{""id"":""msg_1"","
     & """type"":""message"",""role"":""assistant"","
     & """content"":[],""usage"":"
     & "{""input_tokens"":11,""output_tokens"":0}}}" & ASCII.LF & ASCII.LF
     & "event: content_block_start" & ASCII.LF
     & "data: {""type"":""content_block_start"","
     & """index"":0,""content_block"":{""type"":""thinking""}}" & ASCII.LF
     & ASCII.LF & "event: content_block_delta" & ASCII.LF
     & "data: {""type"":""content_block_delta"","
     & """index"":0,""delta"":{""type"":""thinking_delta"","
     & """thinking"":""ponder""}}" & ASCII.LF & ASCII.LF
     & "event: content_block_stop" & ASCII.LF
     & "data: {""type"":""content_block_stop"",""index"":0}" & ASCII.LF
     & ASCII.LF & "event: content_block_start" & ASCII.LF
     & "data: {""type"":""content_block_start"","
     & """index"":1,""content_block"":{""type"":""text""}}" & ASCII.LF
     & ASCII.LF & "event: content_block_delta" & ASCII.LF
     & "data: {""type"":""content_block_delta"","
     & """index"":1,""delta"":{""type"":""text_delta"","
     & """text"":""Hello""}}" & ASCII.LF & ASCII.LF
     & "event: content_block_stop" & ASCII.LF
     & "data: {""type"":""content_block_stop"",""index"":1}" & ASCII.LF
     & ASCII.LF & "event: message_delta" & ASCII.LF
     & "data: {""type"":""message_delta"","
     & """delta"":{""stop_reason"":""end_turn""},"
     & """usage"":{""output_tokens"":7}}" & ASCII.LF & ASCII.LF
     & "event: message_stop" & ASCII.LF & "data: {""type"":""message_stop""}"
     & ASCII.LF & ASCII.LF;
   ...
   Early_Close_SSE_Payload : constant String :=
     ...
     & """type"":""message"",""role"":""assistant"","
     & """content"":[],""usage"":"
     & "{""input_tokens"":4,""output_tokens"":0}}}" & ASCII.LF & ASCII.LF
     & "event: content_block_start" & ASCII.LF
     & "data: {""type"":""content_block_start"","
     ...
     & """text"":""partial""}}" & ASCII.LF & ASCII.LF;
   ...
   procedure Write_Capture (Req : Test_HTTP_Server.Request; Path : String) is
      use GNATCOLL.JSON;
      use Ada.Characters.Handling;

      Root     : constant JSON_Value  := Create_Object;
      Hdrs_JS  : constant JSON_Value  := Create_Object;
      Body_Res : constant Read_Result := Read (To_String (Req.Body_Data));
   ...
   begin
      ...
      for H of Req.Headers loop
         Hdrs_JS.Set_Field
           (To_Lower (To_String (H.Name)), To_String (H.Value));
      end loop;
   ...
   function Parse_Json
     (Data : String; Context : String) return GNATCOLL.JSON.JSON_Value
   is
   ...
   begin
      if not Result.Success then
         raise Constraint_Error
           with Context & ": "
           & GNATCOLL.JSON.Format_Parsing_Error (Result.Error);
      end if;
   ...
   procedure Test_Stream_Thinking_And_Text_Response (T : in out Test) is
      ...
      Provider : LLM.Providers.Anthropic_Messages.Provider :=
        LLM.Providers.Anthropic_Messages.Create
          (Base_Url => "http://127.0.0.1:18773", Api_Key => "test-key");
   ...
   procedure Test_Request_Headers (T : in out Test) is
      ...
      Provider : LLM.Providers.Anthropic_Messages.Provider :=
        LLM.Providers.Anthropic_Messages.Create
          (Base_Url => "http://127.0.0.1:18774", Api_Key => "test-key");
      Messages : constant LLM.Types.Message_Vectors.Vector := Build_Messages;
   ...
   procedure Test_Thinking_Budget_Injection (T : in out Test) is
      ...
      Cases    : constant array (Positive range 1 .. 6)
        of Budget_Case :=
   ...
   begin
      for Index in Cases'Range loop
         declare
            ...
            Provider : LLM.Providers.Anthropic_Messages.Provider :=
              LLM.Providers.Anthropic_Messages.Create
                (Base_Url => "http://127.0.0.1:" & Natural_Image (Port),
                 Api_Key  => "test-key");
            Request  : GNATCOLL.JSON.JSON_Value;
   ...
   is
      ...
      Provider : LLM.Providers.Anthropic_Messages.Provider :=
        LLM.Providers.Anthropic_Messages.Create
          (Base_Url => "http://127.0.0.1:18789", Api_Key => "test-key");
   ...
   procedure Test_Stream_Tool_Use_Response (T : in out Test) is
      ...
      Provider   : LLM.Providers.Anthropic_Messages.Provider :=
        LLM.Providers.Anthropic_Messages.Create
          (Base_Url => "http://127.0.0.1:18790", Api_Key => "test-key");
      Messages   : constant LLM.Types.Message_Vectors.Vector := Build_Messages;
      Sequence   : String_Vectors.Vector;
      Last_Stop  : LLM.Types.Stop_Reason := LLM.Types.Unknown_Stop;
   ...
   procedure Test_Stop_Reason_Mappings (T : in out Test) is
      ...
      Cases    : constant array (Positive range 1 .. 2)
        of Stop_Case :=
        ((Port        => 18_791,
      ...
      for Case_Item of Cases loop
         declare
            Provider     : LLM.Providers.Anthropic_Messages.Provider :=
              LLM.Providers.Anthropic_Messages.Create
                (Base_Url =>
                   "http://127.0.0.1:" & Natural_Image (Case_Item.Port),
                 Api_Key  => "test-key");
            Last_Stop    : LLM.Types.Stop_Reason := LLM.Types.Unknown_Stop;
            Reason       : constant String                           :=
              Ada.Strings.Fixed.Trim (Case_Item.Stop_Reason, Ada.Strings.Both);
            ...
            Stop_Payload : constant String                           :=
              ...
              & """type"":""message"",""role"":""assistant"","
              & """content"":[],""usage"":"
              & "{""input_tokens"":7,""output_tokens"":0}}}" & ASCII.LF
              & ASCII.LF & "event: content_block_start" & ASCII.LF
              & "data: {""type"":""content_block_start"","
              & """index"":0,""content_block"":{""type"":""text""}}" & ASCII.LF
              & ASCII.LF & "event: content_block_delta" & ASCII.LF
              & "data: {""type"":""content_block_delta"","
              & """index"":0,""delta"":"
              & "{""type"":""text_delta"",""text"":""Hi""}}" & ASCII.LF
              & ASCII.LF & "event: content_block_stop" & ASCII.LF
              & "data: {""type"":""content_block_stop""," & """index"":0}"
              & ASCII.LF & ASCII.LF & "event: message_delta" & ASCII.LF
              & "data: {""type"":""message_delta"","
              & """delta"":{""stop_reason"":""" & Reason
              & """},""usage"":{""output_tokens"":2}}" & ASCII.LF & ASCII.LF
              & "event: message_stop" & ASCII.LF
              & "data: {""type"":""message_stop""}" & ASCII.LF & ASCII.LF;
   ...
   procedure Test_Anthropic_HTTP_Error_Propagates (T : in out Test) is
      ...
      Provider      : LLM.Providers.Anthropic_Messages.Provider :=
        LLM.Providers.Anthropic_Messages.Create
          (Base_Url => "http://127.0.0.1:18794", Api_Key => "test-key");
      Messages : constant LLM.Types.Message_Vectors.Vector := Build_Messages;
   ...
   procedure Test_Anthropic_Stream_Terminates_Early (T : in out Test) is
      ...
      Provider : LLM.Providers.Anthropic_Messages.Provider :=
        LLM.Providers.Anthropic_Messages.Create
          (Base_Url => "http://127.0.0.1:18795", Api_Key => "test-key");
      Messages : constant LLM.Types.Message_Vectors.Vector := Build_Messages;
   ...
   procedure Test_Signature_Parsed_From_SSE (T : in out Test) is
      ...
      SSE_With_Signature : constant String :=
        ...
        & """type"":""message"",""role"":""assistant"","
        & """content"":[],""usage"":"
        & "{""input_tokens"":5,""output_tokens"":0}}}" & ASCII.LF & ASCII.LF
        & "event: content_block_start" & ASCII.LF
        & "data: {""type"":""content_block_start"","
        & """index"":0,""content_block"":{""type"":""thinking""}}" & ASCII.LF
        & ASCII.LF & "event: content_block_delta" & ASCII.LF
        & "data: {""type"":""content_block_delta"","
        & """index"":0,""delta"":{""type"":""thinking_delta"","
        & """thinking"":""I reason""}}" & ASCII.LF & ASCII.LF
        & "event: content_block_delta" & ASCII.LF
        & "data: {""type"":""content_block_delta"","
        & """index"":0,""delta"":{""type"":""signature_delta"","
        & """signature"":""sig-xyz-123""}}" & ASCII.LF & ASCII.LF
        & "event: content_block_stop" & ASCII.LF
        & "data: {""type"":""content_block_stop"",""index"":0}" & ASCII.LF
        & ASCII.LF & "event: message_delta" & ASCII.LF
        & "data: {""type"":""message_delta"","
        & """delta"":{""stop_reason"":""end_turn""},"
        & """usage"":{""output_tokens"":3}}" & ASCII.LF & ASCII.LF
        & "event: message_stop" & ASCII.LF
        & "data: {""type"":""message_stop""}" & ASCII.LF & ASCII.LF;
   ...
   procedure Test_Thinking_Block_Serialised_In_Request (T : in out Test) is
      ...
      Minimal_SSE : constant String :=
        ...
        & """type"":""message"",""role"":""assistant"","
        & """content"":[],""usage"":"
        & "{""input_tokens"":10,""output_tokens"":0}}}" & ASCII.LF & ASCII.LF
        & "event: content_block_start" & ASCII.LF
        & "data: {""type"":""content_block_start"","
        & """index"":0,""content_block"":{""type"":""text""}}" & ASCII.LF
        & ASCII.LF & "event: content_block_delta" & ASCII.LF
        & "data: {""type"":""content_block_delta"","
        & """index"":0,""delta"":{""type"":""text_delta"","
        & """text"":""OK""}}" & ASCII.LF & ASCII.LF
        & "event: content_block_stop" & ASCII.LF
        & "data: {""type"":""content_block_stop"",""index"":0}" & ASCII.LF
        & ASCII.LF & "event: message_delta" & ASCII.LF
        & "data: {""type"":""message_delta"","
        & """delta"":{""stop_reason"":""end_turn""},"
        & """usage"":{""output_tokens"":1}}" & ASCII.LF & ASCII.LF
        & "event: message_stop" & ASCII.LF
        & "data: {""type"":""message_stop""}" & ASCII.LF & ASCII.LF;

      Provider : LLM.Providers.Anthropic_Messages.Provider :=
        LLM.Providers.Anthropic_Messages.Create
          (Base_Url => "http://127.0.0.1:18775", Api_Key => "test-key");
      Messages : constant LLM.Types.Message_Vectors.Vector := Build_History;
   ...
   procedure Test_Tool_Result_Is_Error_Serialised (T : in out Test) is
      ...
      begin
         ...
         Result_Content.Append
           ((Kind        => LLM.Types.Tool_Result_Block,
             ...
             Result_Text =>
               To_Unbounded_String ("invalid JSON arguments for shell tool"),
      ...
      Provider : LLM.Providers.Anthropic_Messages.Provider :=
        LLM.Providers.Anthropic_Messages.Create
          (Base_Url => "http://127.0.0.1:18796", Api_Key => "test-key");
      Messages : constant LLM.Types.Message_Vectors.Vector :=
   ...
   begin
      ...
      declare
         ...
         Tool_Msg    : constant JSON_Value := GNATCOLL.JSON.Get (Msg_Array, 3);
         Content_Arr : constant JSON_Array :=
           Get_Array_Field (Tool_Msg, "content");
         Block : constant JSON_Value := GNATCOLL.JSON.Get (Content_Arr, 1);
      begin
         Assert
   ...
   procedure Test_System_Prompt_Is_Content_Block_Array (T : in out Test) is
      ...
      Capture  : constant String := "/tmp/coyote_anthropic_cache_system.json";
      Provider : LLM.Providers.Anthropic_Messages.Provider :=
        LLM.Providers.Anthropic_Messages.Create
          (Base_Url => "http://127.0.0.1:18797", Api_Key => "test-key");
      Messages : constant LLM.Types.Message_Vectors.Vector := Build_Messages;
   ...
   procedure Test_Cache_Control_On_Last_Tool (T : in out Test) is
      ...
      Capture    : constant String := "/tmp/coyote_anthropic_cache_tools.json";
      Provider   : LLM.Providers.Anthropic_Messages.Provider :=
        LLM.Providers.Anthropic_Messages.Create
          (Base_Url => "http://127.0.0.1:18798", Api_Key => "test-key");
      Messages   : constant LLM.Types.Message_Vectors.Vector := Build_Messages;
      Tools_Json : constant String                           :=
        "[{""name"":""read"",""description"":""read file"","
        & """input_schema"":{""type"":""object"","
   ...
   begin
      ...
      declare
         ...
         Captured : constant JSON_Value :=
           Parse_Json (Read_File (Capture), "captured request");
         Body_Val : constant JSON_Value := Get_Object_Field (Captured, "body");
         Tools    : constant JSON_Array := Get_Array_Field (Body_Val, "tools");
      begin
         Assert (Length (Tools) = 2, "tools array should have 2 elements");
   ...
   procedure Test_Cache_Control_On_Last_User_Message (T : in out Test) is
      ...
      Capture : constant String := "/tmp/coyote_anthropic_cache_user_msg.json";
      Provider : LLM.Providers.Anthropic_Messages.Provider :=
        LLM.Providers.Anthropic_Messages.Create
          (Base_Url => "http://127.0.0.1:18799", Api_Key => "test-key");

      --  Two-user-message history: first without cache_control, second with.
      function Build_Two_User_Messages return LLM.Types.Message_Vectors.Vector
      is
         Messages  : LLM.Types.Message_Vectors.Vector;
   ...
   begin
      ...
      begin
         ...
         declare
            ...
            C1 : constant JSON_Array := Get_Array_Field (Msg_1, "content");
         ...
         declare
            ...
            C2 : constant JSON_Array := Get_Array_Field (Msg_2, "content");
   ...
   procedure Test_Tool_Result_Image_Serialised (T : in out Test) is
      ...
      Provider : LLM.Providers.Anthropic_Messages.Provider :=
        LLM.Providers.Anthropic_Messages.Create
          (Base_Url => "http://127.0.0.1:18803", Api_Key => "test-key");
      Messages : constant LLM.Types.Message_Vectors.Vector :=
   ...
   begin
      ...
      declare
         ...
         Inner_Arr : constant JSON_Array := Get_Array_Field (Block, "content");
         Image_Block : constant JSON_Value := GNATCOLL.JSON.Get (Inner_Arr, 1);
   ...
   package LLM_Anthropic_Messages_Caller is new AUnit.Test_Caller
     (LLM_Anthropic_Messages_Tests.Test);
      ...
      Result.Add_Test
        (LLM_Anthropic_Messages_Caller.Create
           ("LLM.Anthropic_Messages injects the correct thinking budget",
            LLM_Anthropic_Messages_Tests.Test_Thinking_Budget_Injection'
              Access));
      Result.Add_Test
        (LLM_Anthropic_Messages_Caller.Create
           ("LLM.Anthropic_Messages encodes compaction summaries as user",
            LLM_Anthropic_Messages_Tests
              .Test_Compaction_Summary_Encodes_As_User_Anthropic'
              Access));
      Result.Add_Test
        (LLM_Anthropic_Messages_Caller.Create
           ("LLM.Anthropic_Messages streams tool_use blocks",
            LLM_Anthropic_Messages_Tests.Test_Stream_Tool_Use_Response'
              Access));
      Result.Add_Test
        (LLM_Anthropic_Messages_Caller.Create
           ("LLM.Anthropic_Messages maps alternate stop reasons",
            LLM_Anthropic_Messages_Tests.Test_Stop_Reason_Mappings'Access));
      Result.Add_Test
        (LLM_Anthropic_Messages_Caller.Create
           ("LLM.Anthropic_Messages uses x-api-key for Anthropic-style URLs",
            LLM_Anthropic_Messages_Tests.Test_Anthropic_Uses_X_Api_Key_Header'
              Access));
      Result.Add_Test
        (LLM_Anthropic_Messages_Caller.Create
           ("LLM.Anthropic_Messages propagates HTTP errors",
            LLM_Anthropic_Messages_Tests.Test_Anthropic_HTTP_Error_Propagates'
              Access));
      Result.Add_Test
        (LLM_Anthropic_Messages_Caller.Create
           ("LLM.Anthropic_Messages finalizes early-terminated streams",
            LLM_Anthropic_Messages_Tests
              .Test_Anthropic_Stream_Terminates_Early'
              Access));
      Result.Add_Test
        (LLM_Anthropic_Messages_Caller.Create
           ("LLM.Anthropic_Messages captures signature from signature_delta",
            LLM_Anthropic_Messages_Tests.Test_Signature_Parsed_From_SSE'
              Access));
      Result.Add_Test
        (LLM_Anthropic_Messages_Caller.Create
      ...
      Result.Add_Test
        (LLM_Anthropic_Messages_Caller.Create
           ("Anthropic last tool has cache_control breakpoint",
            LLM_Anthropic_Messages_Tests.Test_Cache_Control_On_Last_Tool'
              Access));
      Result.Add_Test
        (LLM_Anthropic_Messages_Caller.Create
      ...
      Result.Add_Test
        (LLM_Anthropic_Messages_Caller.Create
           ("Anthropic tool_result with Is_Error includes is_error field",
            LLM_Anthropic_Messages_Tests.Test_Tool_Result_Is_Error_Serialised'
              Access));
      Result.Add_Test
        (LLM_Anthropic_Messages_Caller.Create
           ("Anthropic tool_result image block uses base64 source format",
            LLM_Anthropic_Messages_Tests.Test_Tool_Result_Image_Serialised'
              Access));
