...
package body Coyote_GUI_Mnemonics_Tests is
   ...
   begin
      ...
      Assert
        (Key ("Plain label") = Character'Val (0),
   ...
   begin
      Assert
        (Key ("File __name _Open") = 'o',
         "escaped underscores are not mnemonic markers");
      Assert
        (Key ("Trailing_") = Character'Val (0),
   ...
   begin
      ...
      Assert (Rejected, "a duplicate mnemonic is rejected within one context");
   ...
   begin
      ...
      Assert
        (Key ("Cance_l") = 'l', "Sandbox Cancel uses a unique File-menu key");
   end Test_Current_UI_Context_Allocations;

   package Coyote_GUI_Mnemonics_Caller is new AUnit.Test_Caller
     (Coyote_GUI_Mnemonics_Tests.Test);
   ...
   begin
      Result.Add_Test
        (Coyote_GUI_Mnemonics_Caller.Create
      ...
      Result.Add_Test
        (Coyote_GUI_Mnemonics_Caller.Create
           ("Coyote.GUI.Mnemonics ignores escaped underscores",
            Coyote_GUI_Mnemonics_Tests.Test_Ignores_Escaped_Underscores'
              Access));
      Result.Add_Test
        (Coyote_GUI_Mnemonics_Caller.Create
           ("Coyote.GUI.Mnemonics rejects duplicate context key",
            Coyote_GUI_Mnemonics_Tests.Test_Rejects_Duplicate_Context_Key'
              Access));
      Result.Add_Test
        (Coyote_GUI_Mnemonics_Caller.Create
           ("Coyote.GUI.Mnemonics separates contexts",
            Coyote_GUI_Mnemonics_Tests.Test_Allows_Key_In_Separate_Context'
              Access));
      Result.Add_Test
        (Coyote_GUI_Mnemonics_Caller.Create
           ("Coyote.GUI.Mnemonics protects current UI allocations",
            Coyote_GUI_Mnemonics_Tests.Test_Current_UI_Context_Allocations'
              Access));
      return Result;
   end Suite;
