...
package body Coyote_GUI_Prompt_Queue_Tests is
   ...
   begin
      ...
      Assert
        (Got.Kind = Set_Preferences,
         "preference item kind should survive queue transport");
      Assert
        (To_String (Got.Preferences.Provider) = "openrouter",
         "provider should survive queue transport");
      Assert
        (To_String (Got.Preferences.Model_Id) = "test/model",
         "model ID should survive queue transport");
      Assert
        (Got.Preferences.Thinking = LLM.Providers.High,
         "thinking level should survive queue transport");
      Assert
        (To_String (Got.Preferences.Sandbox) = "restricted",
         "sandbox profile should survive queue transport");
      Assert
        (To_String (Got.Preferences.Subagent_Provider) = "openrouter",
      ...
      begin
         ...
         Queue.Dequeue (Got);
         Assert
           (Got.Preferences.Skill_Paths.Length = 2,
            "skill paths should survive queue transport");
         Assert
           (Got.Preferences.Skill_Paths.Element (1) = "/one/skills",
            "first skill path should survive queue transport");
         Assert
           (Got.Preferences.Skill_Paths.Element (2) = "/two/skills",
   ...
   begin
      Queue.Enqueue
        ((Kind            => Set_Subagent_Model,
          ...
          Override_Spec   =>
            To_Unbounded_String ("openrouter/anthropic/claude-haiku")));
      Queue.Dequeue (Got);
      Assert
        (Got.Kind = Set_Subagent_Model,
      ...
      Assert
        (To_String (Got.Override_Spec) = "openrouter/anthropic/claude-haiku",
      ...
      Assert
        (Length (Got.Override_Spec) = 0,
         "empty override spec should represent an explicit clear");
   end Test_Set_Subagent_Model_Round_Trips;

   package Coyote_GUI_Prompt_Queue_Caller is new AUnit.Test_Caller
     (Coyote_GUI_Prompt_Queue_Tests.Test);
      ...
      Result.Add_Test
        (Coyote_GUI_Prompt_Queue_Caller.Create
           ("Coyote.GUI.Prompt_Queue reports acceptance",
            Coyote_GUI_Prompt_Queue_Tests.Test_Enqueue_Reports_Acceptance'
              Access));
      Result.Add_Test
        (Coyote_GUI_Prompt_Queue_Caller.Create
           ("Coyote.GUI.Prompt_Queue preserves target identity",
            Coyote_GUI_Prompt_Queue_Tests.Test_Target_Agent_Id_Round_Trips'
              Access));
      Result.Add_Test
        (Coyote_GUI_Prompt_Queue_Caller.Create
           ("Coyote.GUI.Prompt_Queue rejects overflow",
            Coyote_GUI_Prompt_Queue_Tests.Test_Enqueue_Rejects_Overflow'
              Access));
      Result.Add_Test
        (Coyote_GUI_Prompt_Queue_Caller.Create
           ("Coyote.GUI.Prompt_Queue Set_Subagent_Model round trips",
            Coyote_GUI_Prompt_Queue_Tests.Test_Set_Subagent_Model_Round_Trips'
              Access));
