...
package body Coyote_SQC_Workspace_Tests is
   ...
   procedure Load_All_Ignored (Path : String; Workspace : out Workspace_Record)
   is
      VF       : Natural;
      Migrated : Boolean;
   begin
   ...
   begin
      ...
      W_Out.Comments.Append
        ((Comment_Id => To_Unbounded_String ("cmt-001"),
          ...
          Timestamp  => Ada.Calendar.Time_Of (2_025, 1, 15, 43_200.0),
      ...
      Assert
        (To_String (W_In.Source_Directories.Element (1))
         = "/home/user/project",
         "Source_Directories[1] round-trip failed");
      Assert (W_In.Model_Filter.Length = 1, "Model_Filter length mismatch");
      Assert
   ...
   begin
      ...
      exception
         when Coyote_SQC.Workspace.Workspace_Error =>
            Raised := True;
      end;
   ...
   procedure Test_UUID_Deduplication (T : in out Test) is
      pragma Unreferenced (T);
      Path : constant String :=
        Ada.Directories.Current_Directory & "/fixtures/sqc/tmp_dedup.sqcw";
      W    : Workspace_Record;
   ...
   begin
      ...
      Assert
        (ID (20) = '8' or else ID (20) = '9' or else ID (20) = 'a'
         or else ID (20) = 'b',
   ...
   begin
      Assert
        (ID1 /= ID2, "Two New_UUID calls should not return the same value");
   ...
   begin
      ...
      Assert
        (Coyote_SQC.Workspace.Chart_Settings (W_In, Turn_Tokens_Xbar).Transform
           .Lambda_Source
         = Robust_Auto,
   ...
   begin
      ...
      Assert
        (Coyote_SQC.Workspace.Chart_Settings (W, Session_Input_Tokens_I)
           .Transform
           .Kind
         = None,
   ...
   begin
      ...
      declare
         E_Cfg : constant Chart_Settings_Record :=
           Coyote_SQC.Workspace.Chart_Settings (W, Session_Input_Tokens_EWMA);
      begin
         Assert
   ...
   begin
      ...
      Assert (VF = 10, "Version should be 10; got " & Natural'Image (VF));
      Assert (not Migrated, "No migration expected for a v8 workspace");
      declare
         I_Cfg : constant Chart_Settings_Record :=
           Coyote_SQC.Workspace.Chart_Settings (W_In, Session_Input_Tokens_I);
      begin
   ...
   begin
      ...
      Assert (W_In.Log_Y_Mode, "Log_Y_Mode should be True after round-trip");
   ...
   begin
      ...
      declare
         ...
         Mig2   : Boolean;
      begin
         W_Out2.Workspace_Id            := To_Unbounded_String ("aad-ws-002");
         W_Out2.Name := To_Unbounded_String ("All Dirs False Test");
         ...
         Assert
           (not W_In2.Analyze_All_Directories,
   ...
   procedure Test_Quantile_Bonferroni_Default (T : in out Test) is
      ...
      Path : constant String :=
        Ada.Directories.Current_Directory
        & "/fixtures/sqc/tmp_bonf_default.sqcw";
   begin
      ...
      begin
         ...
         Ada.Text_IO.Put_Line
           (File,
            "{" & """version"": 10," & """workspaceId"": ""bonf-def-001"","
      ...
      begin
         ...
         Ada.Directories.Delete_File (Path);
         Assert
           (W_In.Quantile_Bonferroni,
            "Quantile_Bonferroni should default to True"
            & " when field absent from workspace file");
      end;
   end Test_Quantile_Bonferroni_Default;

   package SQC_Workspace_Caller is new AUnit.Test_Caller
     (Coyote_SQC_Workspace_Tests.Test);
      ...
      Result.Add_Test
        (SQC_Workspace_Caller.Create
           ("Turn Count Box-Cox: config round-trips through workspace",
            Coyote_SQC_Workspace_Tests.Test_Turn_Count_Box_Cox_Round_Trip'
              Access));
      Result.Add_Test
        (SQC_Workspace_Caller.Create
           ("Turn Count Box-Cox: v4 workspace loads default (disabled)",
            Coyote_SQC_Workspace_Tests.Test_V4_Loads_Turn_Count_Defaults'
              Access));
      Result.Add_Test
        (SQC_Workspace_Caller.Create
           ("Robust estimation: Robust_Median survives workspace round-trip",
            Coyote_SQC_Workspace_Tests.Test_Estimation_Method_Round_Trip'
              Access));
      Result.Add_Test
        (SQC_Workspace_Caller.Create
           ("Robust estimation: v5 workspace loads Classical default",
            Coyote_SQC_Workspace_Tests.Test_V5_Loads_Classical_Default'
              Access));
      Result.Add_Test
        (SQC_Workspace_Caller.Create
           ("SQC workspace: Anscombe transform round-trips through save/load",
            Coyote_SQC_Workspace_Tests.Test_Anscombe_Transform_Round_Trip'
              Access));
      Result.Add_Test
        (SQC_Workspace_Caller.Create
           ("SQC workspace: logYMode round-trips through workspace save/load",
            Coyote_SQC_Workspace_Tests.Test_Log_Y_Mode_Round_Trip'Access));
      Result.Add_Test
        (SQC_Workspace_Caller.Create
           ("SQC workspace: Analyze_All_Directories round-trips through save/load",
            Coyote_SQC_Workspace_Tests.Test_Analyze_All_Directories_Round_Trip'
              Access));
      Result.Add_Test
        (SQC_Workspace_Caller.Create
           ("Quantile_Bonferroni round-trip",
            Coyote_SQC_Workspace_Tests.Test_Quantile_Bonferroni_Round_Trip'
              Access));
      Result.Add_Test
        (SQC_Workspace_Caller.Create
           ("Quantile_Bonferroni defaults to True when absent",
            Coyote_SQC_Workspace_Tests.Test_Quantile_Bonferroni_Default'
              Access));
