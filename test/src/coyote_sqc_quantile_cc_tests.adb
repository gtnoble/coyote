...
with Coyote_SQC.Statistics.Quantile_CC; use Coyote_SQC.Statistics.Quantile_CC;
...
package body Coyote_SQC_Quantile_CC_Tests is
   ...
   procedure Check_Quantile
     (Label   : String;
      Got     : Long_Float;
      Want    : Long_Float;
      Epsilon : Long_Float := 1.0e-10)
   is
   begin
      Assert
        (abs (Got - Want) <= Epsilon,
         Label & ": got " & Got'Image & " want " & Want'Image);
   end Check_Quantile;
   ...
   begin
      ...
      declare
         ...
         RNG_State : Long_Long_Integer          :=
           Long_Long_Integer (abs (Seed)) + Long_Long_Integer (N_I);
      ...
      begin
         ...
         for Comp in Quantile_Index loop
            Dist (Comp).Reserve_Capacity (Ada.Containers.Count_Type (Small_B));
         end loop;
         for B in 1 .. Small_B loop
            ...
            begin
               for J in 1 .. N_I loop
                  RNG_State :=
                    (RNG_State * 1_103_515_245 + 12_345) mod Modulus;
                  if RNG_State = 0 then
                     RNG_State := 1;
                  end if;
                  declare
                     Pick : constant Natural :=
                       1 + Natural (RNG_State mod Long_Long_Integer (Length));
                  begin
   ...
   is
      ...
      Rnk    : constant Natural :=
        Natural'Max
          (1, Natural (Long_Float'Floor (0.000_27 * Long_Float (Small_B))));
   begin
      for Comp in Quantile_Index loop
         declare
   ...
   begin
      ...
      for Comp in Quantile_Index loop
         ...
         Assert
           (Lims (Comp).UCL >= Lims (Comp).CL, "UCL >= CL for " & Comp'Image);
         Assert
           (Lims (Comp).CL >= Lims (Comp).LCL, "CL >= LCL for " & Comp'Image);
      end loop;
   ...
   begin
      ...
      for Comp in Quantile_Index loop
         Assert
           (Natural (Dist (Comp).Length) = Small_B,
   ...
   begin
      ...
      for Comp in Quantile_Index loop
         ...
         begin
            Assert
              (D1 (Comp).Element (0) = D2 (Comp).Element (0),
               "first element mismatch");
            Assert
              (D1 (Comp).Element (L / 2) = D2 (Comp).Element (L / 2),
               "middle element mismatch");
            Assert
              (D1 (Comp).Element (L - 1) = D2 (Comp).Element (L - 1),
   ...
   begin
      ...
      declare
         Lims : constant Quantile_Limits_Array := Small_Extract_Limits (Dist);
      begin
   ...
   begin
      Vals (Min_Q) := 100.0;
      Assert
        (Session_Is_OOC (Vals, Lims), "one OOC component makes session OOC");
   ...
   begin
      ...
      begin
         ...
         Clear_Cache (Cache);
         Assert
           (Natural (Cache.Entries.Length) = 0,
   ...
   begin
      for I in Quantile_Index loop
         Check_Quantile ("reverse " & I'Image, Q_Reverse (I), Q_Sorted (I));
      end loop;
   ...
   begin
      ...
      Assert (Q (Min_Q) <= Q (Q1), "min <= Q1 for two elements");
      Assert (Q (Q1) <= Q (Median_Q), "Q1 <= median for two elements");
      Assert (Q (Median_Q) <= Q (Q3), "median <= Q3 for two elements");
      Assert (Q (Q3) <= Q (Max_Q), "Q3 <= max for two elements");
   ...
   begin
      ...
      Assert (Q (Min_Q) <= Q (Q1), "min <= Q1 for 50 elements");
      Assert (Q (Q1) <= Q (Median_Q), "Q1 <= median for 50 elements");
      Assert (Q (Median_Q) <= Q (Q3), "median <= Q3 for 50 elements");
      Assert (Q (Q3) <= Q (Max_Q), "Q3 <= max for 50 elements");
   ...
   begin
      ...
      Lims_Interp := Interpolate_Limits (Cache, Pool, Offs, Lens, 3);
      declare
      ...
      for Comp in Quantile_Index loop
         Assert
           (Lims_Interp (Comp).Has_UCL = Lims_Exact (Comp).Has_UCL,
            "anchor: Has_UCL mismatch for " & Comp'Image);
         Assert
           (Lims_Interp (Comp).Has_LCL = Lims_Exact (Comp).Has_LCL,
            "anchor: Has_LCL mismatch for " & Comp'Image);
         Assert
           (abs (Lims_Interp (Comp).CL - Lims_Exact (Comp).CL) <= 1.0e-10,
            "anchor: CL mismatch for " & Comp'Image);
         Assert
           (abs (Lims_Interp (Comp).UCL - Lims_Exact (Comp).UCL) <= 1.0e-10,
            "anchor: UCL mismatch for " & Comp'Image);
         Assert
           (abs (Lims_Interp (Comp).LCL - Lims_Exact (Comp).LCL) <= 1.0e-10,
   ...
   begin
      ...
      Lims_Small := Interpolate_Limits (Cache_Small, Pool, Offs, Lens, 3);
      Lims_Large := Interpolate_Limits (Cache_Large, Pool, Offs, Lens, 10);

      for Comp in Quantile_Index loop
         if Lims_Small (Comp).Has_UCL and then Lims_Large (Comp).Has_UCL then
            declare
         ...
         if Lims_Small (Comp).Has_LCL and then Lims_Large (Comp).Has_LCL then
            declare
               HW_Small : constant Long_Float :=
   ...
   begin
      ...
      Lims := Interpolate_Limits (Cache, Pool, Offs, Lens, 1);

      for Comp in Quantile_Index loop
         Assert
           (Lims (Comp).Has_UCL, "n=1: should have UCL for " & Comp'Image);
         Assert
           (Lims (Comp).Has_LCL, "n=1: should have LCL for " & Comp'Image);
         Assert
           (Lims (Comp).CL > 0.0,
   ...
   procedure Test_Extract_Limits_Bonferroni_Disabled (T : in out Test) is
      pragma Unreferenced (T);
      Dist       : Bootstrap_Distribution;
   ...
   begin
      ...
      for Comp in Quantile_Index loop
         Assert
           (abs (Bonf_Lims (Comp).UCL - Unadj_Lims (Comp).UCL) <= 1.0e-10,
            "Bonferroni limits match unadjusted at Small_B for " & Comp'Image);
      end loop;
      ...
      begin
         ...
         for Comp in Quantile_Index loop
            Assert
              (Unadj_Full (Comp).Has_UCL,
               "unadjusted Has_UCL for " & Comp'Image);
            Assert
              (Unadj_Full (Comp).Has_LCL,
               "unadjusted Has_LCL for " & Comp'Image);
            if Bonf_Full (Comp).Has_UCL and Unadj_Full (Comp).Has_UCL then
               Assert
                 (Unadj_Full (Comp).UCL <= Bonf_Full (Comp).UCL + 1.0e-10,
                  "unadjusted UCL <= Bonferroni UCL for " & Comp'Image);
            end if;
            if Bonf_Full (Comp).Has_LCL and Unadj_Full (Comp).Has_LCL then
               Assert
                 (Unadj_Full (Comp).LCL >= Bonf_Full (Comp).LCL - 1.0e-10,
                  "unadjusted LCL >= Bonferroni LCL for " & Comp'Image);
            end if;
         end loop;
         Assert
   ...
   package SQC_Quantile_CC_Caller is new AUnit.Test_Caller
     (Coyote_SQC_Quantile_CC_Tests.Test);
      ...
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
           ("Quantile CC: Build_Distribution with 3-session pool",
            Coyote_SQC_Quantile_CC_Tests.Test_Build_Distribution_Limits'
              Access));
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
           ("Quantile CC: Build_Distribution single-session pool",
            Coyote_SQC_Quantile_CC_Tests.Test_Build_Distribution_Single'
              Access));
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
           ("Quantile CC: Build_Distribution seed reproducibility",
            Coyote_SQC_Quantile_CC_Tests.Test_Build_Distribution_Seeding'
              Access));
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
           ("Quantile CC: Extract_Limits with known distribution",
            Coyote_SQC_Quantile_CC_Tests.Test_Extract_Limits_Known'Access));
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
      ...
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
           ("Quantile CC: Session_Is_OOC all in-control",
            Coyote_SQC_Quantile_CC_Tests.Test_Session_Is_OOC_All_In'Access));
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
           ("Quantile CC: Session_Is_OOC one component out",
            Coyote_SQC_Quantile_CC_Tests.Test_Session_Is_OOC_One_Out'Access));
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
      ...
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
           ("Quantile CC: cache invalidation clears entries",
            Coyote_SQC_Quantile_CC_Tests.Test_Cache_Invalidation'Access));
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
           ("Quantile CC: sort through quantiles -- reverse input",
            Coyote_SQC_Quantile_CC_Tests.Test_Sort_Through_Quantiles_Reverse'
              Access));
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
           ("Quantile CC: sort through quantiles -- all equal values",
            Coyote_SQC_Quantile_CC_Tests.Test_Sort_Through_Quantiles_All_Equal'
              Access));
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
           ("Quantile CC: sort through quantiles -- two descending",
            Coyote_SQC_Quantile_CC_Tests.Test_Sort_Through_Quantiles_Two_Desc'
              Access));
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
           ("Quantile CC: sort through quantiles -- 50 random-ish",
            Coyote_SQC_Quantile_CC_Tests.Test_Sort_Through_Quantiles_Larger'
              Access));
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
           ("Interpolate_Limits at anchor matches exact",
            Coyote_SQC_Quantile_CC_Tests.Test_Interpolate_Limits_Anchor'
              Access));
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
           ("Interpolate_Limits between anchors shrinks HW",
            Coyote_SQC_Quantile_CC_Tests.Test_Interpolate_Limits_Between'
              Access));
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
           ("Interpolate_Limits n=1 falls back to exact",
            Coyote_SQC_Quantile_CC_Tests.Test_Interpolate_Limits_N1'Access));
      Result.Add_Test
        (SQC_Quantile_CC_Caller.Create
           ("Extract_Limits with Bonferroni disabled uses unadjusted ranks",
            Coyote_SQC_Quantile_CC_Tests
              .Test_Extract_Limits_Bonferroni_Disabled'
              Access));
