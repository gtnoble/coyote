...
begin
   ...
   if Ada.Command_Line.Argument_Count > 1 then
      Ada.Text_IO.Put_Line
        (Ada.Text_IO.Standard_Error, "usage: coyote_test [name-prefix]");
   ...
   if Natural
       (AUnit.Test_Results.Test_Count (AUnit.Test_Results.Result (Results)))
     = 0
   then
      Ada.Text_IO.Put_Line
        (Ada.Text_IO.Standard_Error, "error: test filter matched no tests");
      Ada.Command_Line.Set_Exit_Status (Ada.Command_Line.Failure);
