...
package body Coyote_App_Agent_Registry_Tests is
   ...
   begin
      Assert
        (Register_Root (R, Root_Id, "session-root", "main"),
         "root registration must succeed");
      Agent_Data := Agent_At (R, 1);
      Assert (Agent_Count (R) = 1, "root must be the first record");
      Assert
        (Agent_Data.Runtime_Id = Root_Id, "root identity must be retained");
      Assert
        (Is_Empty (Agent_Data.Parent_Runtime_Id),
         "root must not have a parent");
      Assert
        (To_String (Agent_Data.Durable_Session_Id) = "session-root",
         "durable session identity must be retained");
      Assert
        (To_String (Agent_Data.Label) = "main", "root label must be retained");
   ...
   begin
      ...
      Assert
        (Register_Child (R, Child_Id, Root_Id, "session-child", "worker"),
         "child registration must succeed");
      Agent_Data := Get_Agent (R, Child_Id);
      Assert
        (Agent_Data.Parent_Runtime_Id = Root_Id,
         "child parent identity must be retained");
      Assert
        (Agent_Data.Endpoint = RPC_Endpoint, "child endpoint should be RPC");
      Assert (Child_Count (R, Root_Id) = 1, "parent must report one child");
      Assert
        (Child_At (R, Root_Id, 1) = Child_Id,
   ...
   begin
      ...
      Assert
        (Agent_Data.Endpoint = Local_Endpoint,
      ...
      Agent_Data := Get_Agent (R, Child_Id);
      Assert
        (Agent_Data.Endpoint = RPC_Endpoint,
   ...
   begin
      Assert (Register_Root (R, Root_Id), "root registration must succeed");
      Assert
        (Register_Child (R, Child_Id, Root_Id),
         "child registration must succeed");
      Assert
        (Register_Child (R, Grandchild_Id, Child_Id),
         "grandchild registration must succeed");
      Assert
        (Child_Count (R, Root_Id) = 1, "root must have only its direct child");
      Assert
        (Child_Count (R, Child_Id) = 1,
         "child must have its direct grandchild");
      Assert
        (Child_At (R, Child_Id, 1) = Grandchild_Id,
   ...
   begin
      Assert
        (Root_Id /= Create_Agent_Id ("session-root"),
         "runtime and durable identities must be distinct values");
      Assert
        (Register_Root (R, Root_Id, "session-root"),
         "root registration must succeed");
      Agent_Data := Get_Agent (R, Root_Id);
      Assert
        (To_String (Agent_Data.Durable_Session_Id) = "session-root",
   ...
   begin
      Assert
        (Register_Root (R, Root_Id, Status => Running),
         "root registration must succeed");
      Assert
        (Register_Child (R, Child_Id, Root_Id, Status => Running),
         "child registration must succeed");
      Assert (Select_Agent (R, Child_Id), "live child selection must succeed");
      Assert
        (Selected_Agent (R) = Child_Id, "selected identity must be retained");
      Assert (Has_Selection (R), "registry must report a selection");
      Assert
        (Can_Control (R, Child_Id),
   ...
   begin
      Assert
        (Register_Root (R, Root_Id, Status => Ready),
         "ready root registration must succeed");
      Assert
        (Can_Control (R, Root_Id), "ready agent must accept live controls");
   ...
   begin
      ...
      Assert
        (Register_Child (R, Child_Id, Root_Id),
         "child registration must succeed");
      Assert
        (Can_Control (R, Child_Id), "starting child must accept Stop control");
   ...
   begin
      ...
      Assert
        (Register_Child (R, Child_Id, Root_Id),
         "child registration must succeed");
      Assert
        (Set_Status (R, Child_Id, Running), "running transition must succeed");
      Assert (Can_Control (R, Child_Id), "running child must accept control");
      Assert
        (Set_Status (R, Child_Id, Paused), "paused transition must succeed");
      Assert (Can_Control (R, Child_Id), "paused child must accept control");
      Assert
        (Set_Status (R, Child_Id, Ready), "ready transition must succeed");
      Assert
        (Can_Control (R, Child_Id), "ready child must accept Stop control");
      Assert
        (Set_Status (R, Child_Id, Aborted), "aborted transition must succeed");
      Assert
        (not Can_Control (R, Child_Id), "aborted child must reject control");
   ...
   begin
      Assert (Register_Root (R, Root_Id), "root registration must succeed");
      Assert
        (Set_Durable_Session_Id (R, Root_Id, "session-42"),
         "durable session update must succeed");
      Agent_Data := Get_Agent (R, Root_Id);
      Assert
        (To_String (Agent_Data.Durable_Session_Id) = "session-42",
   ...
   begin
      Assert
        (Register_Root (R, Root_Id, Status => Completed),
         "terminal root registration must succeed");
      Assert
        (Select_Agent (R, Root_Id),
         "terminal agent must remain selectable for review");
      Assert
        (Selected_Agent (R) = Root_Id, "terminal selection must be retained");
      Assert
        (Agent_Count (R) = 1, "terminal record must remain in the registry");
   ...
   begin
      Assert
        (Register_Root (R, Root_Id, Status => Failed),
         "failed root registration must succeed");
      Assert
        (not Can_Control (R, Root_Id),
         "terminal agent must reject live controls");
      Assert
        (Set_Status (R, Root_Id, Running), "status transition must succeed");
      Assert
        (Can_Control (R, Root_Id), "running agent must accept live controls");
   ...
   begin
      ...
      Assert
        (Agent_Count (R) = 1, "rejected child must not alter the registry");
   ...
   begin
      Assert (Register_Root (R, Root_Id), "root registration must succeed");
      Assert
        (not Register_Root (R, Root_Id),
         "second root registration must be rejected");
      Assert
        (not Register_Child (R, Root_Id, Root_Id),
   ...
   begin
      ...
      Assert
        (not Has_Agent (R, Root_Id), "cleared agent must no longer be found");
   end Test_Clear_Removes_All_Records;

   package Agent_Registry_Caller is new AUnit.Test_Caller
     (Coyote_App_Agent_Registry_Tests.Test);
   ...
   begin
      Result.Add_Test
        (Agent_Registry_Caller.Create
           ("Agent registry registers the main root",
            Coyote_App_Agent_Registry_Tests.Test_Register_Main_Agent_As_Root'
              Access));
      Result.Add_Test
        (Agent_Registry_Caller.Create
           ("Agent registry registers a child under its parent",
            Coyote_App_Agent_Registry_Tests.Test_Register_Child_Under_Parent'
              Access));
      Result.Add_Test
        (Agent_Registry_Caller.Create
           ("Agent registry records local and RPC endpoints",
            Coyote_App_Agent_Registry_Tests.Test_Register_Agent_Endpoint_Kind'
              Access));
      Result.Add_Test
        (Agent_Registry_Caller.Create
           ("Agent registry preserves recursive descendants",
            Coyote_App_Agent_Registry_Tests
              .Test_Register_Recursive_Descendants'
              Access));
      Result.Add_Test
        (Agent_Registry_Caller.Create
           ("Agent registry separates runtime and durable identity",
            Coyote_App_Agent_Registry_Tests.Test_Runtime_Identity_Is_Separate'
              Access));
      Result.Add_Test
        (Agent_Registry_Caller.Create
           ("Agent registry selects a live agent",
            Coyote_App_Agent_Registry_Tests.Test_Select_Live_Agent'Access));
      Result.Add_Test
        (Agent_Registry_Caller.Create
           ("Agent registry accepts controls while ready",
            Coyote_App_Agent_Registry_Tests.Test_Ready_Agent_Accepts_Control'
              Access));
      Result.Add_Test
        (Agent_Registry_Caller.Create
           ("Agent registry accepts controls while starting",
            Coyote_App_Agent_Registry_Tests
              .Test_Starting_Agent_Accepts_Control'
              Access));
      Result.Add_Test
        (Agent_Registry_Caller.Create
           ("Agent registry preserves live status control policy",
            Coyote_App_Agent_Registry_Tests.Test_Live_Status_Transitions'
              Access));
      Result.Add_Test
        (Agent_Registry_Caller.Create
           ("Agent registry updates durable session identity",
            Coyote_App_Agent_Registry_Tests.Test_Durable_Session_Id_Update'
              Access));
      Result.Add_Test
        (Agent_Registry_Caller.Create
           ("Agent registry retains terminal selection",
            Coyote_App_Agent_Registry_Tests
              .Test_Terminal_Agent_Remains_Selectable'
              Access));
      Result.Add_Test
        (Agent_Registry_Caller.Create
           ("Agent registry rejects terminal controls",
            Coyote_App_Agent_Registry_Tests
              .Test_Terminal_Agent_Rejects_Control'
              Access));
      Result.Add_Test
        (Agent_Registry_Caller.Create
           ("Agent registry rejects unknown parents",
            Coyote_App_Agent_Registry_Tests.Test_Unknown_Parent_Is_Rejected'
              Access));
      Result.Add_Test
        (Agent_Registry_Caller.Create
           ("Agent registry rejects duplicate runtime identities",
            Coyote_App_Agent_Registry_Tests
              .Test_Duplicate_Runtime_Id_Is_Rejected'
              Access));
      Result.Add_Test
        (Agent_Registry_Caller.Create
           ("Agent registry clears records and selection",
            Coyote_App_Agent_Registry_Tests.Test_Clear_Removes_All_Records'
              Access));
