...
package body Coyote_SQC_Histogram_Tests is
   ...
   procedure Test_Bins_N2 (T : in out Test) is
      ...
      Vals  : constant Long_Float_Array :=
        (1.0,
         2.0);
      N     : Positive;
      B_Min : Long_Float;
   ...
   begin
      Compute_Bins (Vals, N, BMin, BWid, Cnts);
      Assert (N = 1, "FD IQR=0: should fall back to 1 bin, got" & N'Image);
      Assert
        (Cnts (1) = 5,
         "FD IQR=0: all 5 values should be in bin 1, got" & Cnts (1)'Image);
   ...
   begin
      ...
      Assert
        (Cnts (1) = 3, "all-equal: count should be 3, got" & Cnts (1)'Image);
   ...
   begin
      ...
      Assert (Cnts (1) = 1, "n=1: count should be 1, got" & Cnts (1)'Image);
   ...
   package SQC_Histogram_Caller is new AUnit.Test_Caller
     (Coyote_SQC_Histogram_Tests.Test);
