...
package body Coyote_App_Agent_RPC_Tests is
   ...
   procedure Expect_RPC_Error (Text : String; Message : String) is
      Ignored : Frame;
   begin
      Ignored := Decode (Text);
   ...
   begin
      ...
      Assert
        (To_String (Output.Agent_Id) = "worker-7",
         "handshake agent identity must round-trip");
      Assert
        (To_String (Output.Parent_Agent_Id) = "root",
         "handshake parent identity must round-trip");
      Assert
        (To_String (Output.Session_Id) = "session-7",
         "handshake session identity must round-trip");
      Assert
        (To_String (Output.Label) = "search worker",
   ...
   begin
      ...
      Assert (Output.Event_Name = Text_Delta, "event name must round-trip");
      Assert
        (To_String (Output.Payload_Json) = "{""text"":""fragment""}",
         "event payload must round-trip");
   end Test_Event_Round_Trip;

   procedure Test_Context_Update_Event_Round_Trip (T : in out Test) is
      pragma Unreferenced (T);
      Input  : constant Frame :=
        Make_Event
          (Agent_Id     => "worker-7",
           ...
           Payload_Json =>
             "{""contextTokens"":42000,""contextWindow"":200000}");
      Output : constant Frame := Decode (Encode (Input));
   begin
      Assert
        (Output.Event_Name = Context_Update,
         "context update event name must round-trip");
      Assert
        (To_String (Output.Payload_Json) = To_String (Input.Payload_Json),
   ...
   begin
      ...
      Assert
        (To_String (Output.Request_Id) = "cmd-19",
         "command request identity must round-trip");
      Assert (Output.Command_Name = Steer, "command name must round-trip");
      Assert
        (To_String (Output.Payload_Json) = "{""text"":""change direction""}",
   ...
   procedure Test_Stop_Command_Round_Trip (T : in out Test) is
      ...
      Output : constant Frame := Decode (Encode (Input));
   begin
      Assert (Output.Kind = Command, "Stop command kind must round-trip");
      Assert (Output.Command_Name = Stop, "Stop command name must round-trip");
      Assert
        (To_String (Output.Request_Id) = "stop-19",
   ...
   begin
      Assert
        (Output.Command_Name = Abort_Tool,
         "Abort_Tool command name must round-trip");
      Assert
        (Ada.Strings.Fixed.Index
           (To_String (Output.Payload_Json), """toolId"":""call-2""")
         > 0,
   ...
   begin
      Assert
        (Output.Command_Name = Set_Sandbox,
         "Set_Sandbox command name must round-trip");
      Assert
        (To_String (Output.Payload_Json) = "{""profile"":""project-write""}",
   ...
   begin
      ...
      Assert
        (To_String (Output.Error_Text) = "provider failed",
         "terminal error must round-trip");
      Assert (Output.Last_Sequence = 27, "terminal sequence must round-trip");
   ...
   begin
      Assert
        (To_String (Output.Payload_Json) = "{""text"":""line\n""}",
   ...
   begin
      Assert
        (not Try_Decode ("{bad", Value, Status, Error),
   ...
   package Agent_RPC_Caller is new AUnit.Test_Caller
     (Coyote_App_Agent_RPC_Tests.Test);
      ...
      Result.Add_Test
        (Agent_RPC_Caller.Create
           ("Agent RPC Stop command round-trips",
            Coyote_App_Agent_RPC_Tests.Test_Stop_Command_Round_Trip'Access));
      Result.Add_Test
        (Agent_RPC_Caller.Create
           ("Agent RPC Abort_Tool command round-trips",
            Coyote_App_Agent_RPC_Tests.Test_Abort_Tool_Command_Round_Trip'
              Access));
      Result.Add_Test
        (Agent_RPC_Caller.Create
           ("Agent RPC Abort_Tool rejects invalid payloads",
            Coyote_App_Agent_RPC_Tests
              .Test_Abort_Tool_Command_Rejects_Invalid_Payload'
              Access));
      Result.Add_Test
        (Agent_RPC_Caller.Create
           ("Agent RPC Set_Sandbox command round-trips",
            Coyote_App_Agent_RPC_Tests.Test_Set_Sandbox_Command_Round_Trip'
              Access));
      Result.Add_Test
        (Agent_RPC_Caller.Create
      ...
      Result.Add_Test
        (Agent_RPC_Caller.Create
           ("Agent RPC encoding has no trailing newline",
            Coyote_App_Agent_RPC_Tests.Test_Encode_Has_No_Trailing_Newline'
              Access));
      Result.Add_Test
        (Agent_RPC_Caller.Create
