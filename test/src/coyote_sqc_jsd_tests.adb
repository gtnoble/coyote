...
package body Coyote_SQC_JSD_Tests is
   ...
   begin
      Assert
        (Coyote_SQC.Statistics.JSD.Token_Count ("shell", "") = 1,
   ...
   begin
      Assert
        (Coyote_SQC.Statistics.JSD.Token_Count ("run test", "") = 2,
   ...
   begin
      Assert
        (Coyote_SQC.Statistics.JSD.Token_Count ("", "") = 0,
   ...
   begin
      Assert
        (S_Diff < S_Same,
         ...
         & "identical tool names; Diff=" & Long_Float'Image (S_Diff) & " Same="
         & Long_Float'Image (S_Same));
   ...
   function Make_TC (Name : String; Args : String) return Tool_Call_Record is
      TC : Tool_Call_Record;
   begin
   ...
   begin
      ...
      M :=
        Coyote_SQC.Metrics.Compute
          (Session, Coyote_SQC.Metrics.Pricing_Maps.Empty_Map);
      ...
      Assert
        (M.Per_Consecutive_Tool_S.Length = 1,
         "Two identical calls: Per_Consecutive_Tool_S.Length must be 1; got "
         & Ada.Containers.Count_Type'Image (M.Per_Consecutive_Tool_S.Length));
      Assert
        (abs (M.Total_Tool_Call_JSD_S - 4.0) <= Tol,
   ...
   begin
      ...
      Assert
        (abs (M.Total_Tool_Call_JSD_S - Manual) <= Tol,
         "Total_Tool_Call_JSD_S must equal sum of Per_Consecutive_Tool_S; "
         & "Total=" & Long_Float'Image (M.Total_Tool_Call_JSD_S) & " Manual="
         & Long_Float'Image (Manual));
   ...
   begin
      ...
      Assert
        (abs (Params.Grand_Mean - 20.0) <= Tol,
   ...
   begin
      ...
      Assert
        (abs (Params.Mean_MR - 20.0) <= Tol,
         "JSD Sum I excludes Pairs=0: Mean_MR should be 20.0; got "
         & Long_Float'Image (Params.Mean_MR));
   end Test_Estimate_JSD_Sum_Excludes_No_Pairs;

   package SQC_JSD_Caller is new AUnit.Test_Caller (Coyote_SQC_JSD_Tests.Test);
      ...
      Result.Add_Test
        (SQC_JSD_Caller.Create
           ("SQC JSD: Token_Count multi-word tool name",
            Coyote_SQC_JSD_Tests.Test_Token_Count_Multi_Word_Tool_Name'
              Access));
      Result.Add_Test
        (SQC_JSD_Caller.Create
      ...
      Result.Add_Test
        (SQC_JSD_Caller.Create
           ("SQC JSD: Compute_S_Values identical calls pair-level sum non-zero",
            Coyote_SQC_JSD_Tests.Test_S_Values_Identical_Calls_Non_Zero'
              Access));
      Result.Add_Test
        (SQC_JSD_Caller.Create
           ("SQC JSD: Compute_S_Values identical calls sum = 4.0",
            Coyote_SQC_JSD_Tests.Test_S_Values_Identical_Calls_Sum'Access));
      Result.Add_Test
        (SQC_JSD_Caller.Create
           ("SQC JSD: Compute_S_Values one-side absent key pair-level sum equals tool_name-only S_k",
            Coyote_SQC_JSD_Tests.Test_S_Values_One_Side_Absent'Access));
      Result.Add_Test
        (SQC_JSD_Caller.Create
           ("SQC JSD: Compute_S_Values integer-valued key skipped (N_k = 0)",
            Coyote_SQC_JSD_Tests.Test_S_Values_Integer_Key_Skipped'Access));
      Result.Add_Test
        (SQC_JSD_Caller.Create
           ("SQC JSD: Compute_S_Values different tool names give lower pair-level sum",
            Coyote_SQC_JSD_Tests.Test_S_Values_Different_Tool_Names'Access));
      Result.Add_Test
        (SQC_JSD_Caller.Create
           ("SQC JSD: Metrics.Compute N_Consecutive_Tool_Pairs=1 for two calls",
            Coyote_SQC_JSD_Tests.Test_Metrics_JSD_Two_Identical_Calls'Access));
      Result.Add_Test
        (SQC_JSD_Caller.Create
           ("SQC JSD: Metrics.Compute N_Consecutive_Tool_Pairs=0 for one call",
            Coyote_SQC_JSD_Tests.Test_Metrics_JSD_Single_Tool_Call'Access));
      Result.Add_Test
        (SQC_JSD_Caller.Create
      ...
      Result.Add_Test
        (SQC_JSD_Caller.Create
           ("SQC JSD: Estimate_Parameters JSD Sum I Grand_Mean",
            Coyote_SQC_JSD_Tests.Test_Estimate_JSD_Sum_I_Grand_Mean'Access));
      Result.Add_Test
        (SQC_JSD_Caller.Create
           ("SQC JSD: Estimate_Parameters JSD Sum I Mean_MR",
            Coyote_SQC_JSD_Tests.Test_Estimate_JSD_Sum_I_Mean_MR'Access));
      Result.Add_Test
        (SQC_JSD_Caller.Create
           ("SQC JSD: Estimate_Parameters JSD Sum excludes sessions with 0 pairs",
            Coyote_SQC_JSD_Tests.Test_Estimate_JSD_Sum_Excludes_No_Pairs'
              Access));
