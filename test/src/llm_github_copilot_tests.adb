...
package body LLM_GitHub_Copilot_Tests is
   ...
   begin
      if not Parsed.Success then
         raise Constraint_Error
           with "Failed to parse GitHub Copilot catalogue fixture";
      end if;
   ...
   begin
      Write_File
        (Home & "/.coyote/auth.json",
         "{""github-copilot"":{" & """type"":""oauth"","
         & """refresh"":""refresh-token""," & """access"":""expired-token"","
         & """expires"":0}}");
   ...
   begin
      Write_File
        (Home & "/.coyote/github_copilot_models_cache.json",
         ...
         & ",""base_url"":""" & Base_Url & """,""data"":" & Fixture_Data_Array
         & "}");
   end Write_Cache;
   ...
   is
      ...
      Anthropic_SSE : constant String :=
        ...
        & """usage"":{""input_tokens"":1,""output_tokens"":0}}}" & ASCII.LF
        & ASCII.LF & "event: content_block_start" & ASCII.LF
        & "data: {""type"":""content_block_start"",""index"":0,"
        & """content_block"":{""type"":""text""}}" & ASCII.LF & ASCII.LF
        & "event: content_block_delta" & ASCII.LF
        & "data: {""type"":""content_block_delta"",""index"":0,"
        & """delta"":{""type"":""text_delta"",""text"":""Claude""}}" & ASCII.LF
      ...
      OpenAI_SSE : constant String :=
        Build_OpenAI_SSE
          (Text => "GPT", Prompt_Tokens => 1, Completion_Tokens => 1);
      ...
      Base_Was_Set : constant Boolean                      :=
        Ada.Environment_Variables.Exists ("COYOTE_GITHUB_COPILOT_BASE_URL");
      Old_Base_Url : constant String                       :=
        Ada.Environment_Variables.Value ("COYOTE_GITHUB_COPILOT_BASE_URL", "");
   ...
   begin
      ...
      Send_With_Retry
        (P => Provider, Model_Id => Model_Id, Messages => Messages);
   ...
   begin
      ...
      Assert
        (Get_String_Field (Headers, "user-agent") = "GitHubCopilotChat/0.35.0",
      ...
      Assert
        (Get_String_Field (Headers, "copilot-integration-id") = "vscode-chat",
         "Copilot-Integration-Id header should be added");
      Assert
        (Get_String_Field (Headers, "openai-intent") = "conversation-edits",
      ...
      Assert
        (To_String (Last_Text) = "Claude", "Expected Anthropic text delta");
   ...
   is
      ...
      Refresh_SSE : constant String :=
        ...
        & "event: content_block_delta" & ASCII.LF
        & "data: {""type"":""content_block_delta"",""index"":0,"
        & """delta"":{""type"":""text_delta""," & """text"":""Fresh Claude""}}"
        & ASCII.LF & ASCII.LF & "event: content_block_stop" & ASCII.LF
        & "data: {""type"":""content_block_stop"",""index"":0}" & ASCII.LF
        & ASCII.LF & "event: message_delta" & ASCII.LF
        & "data: {""type"":""message_delta"","
        & """delta"":{""stop_reason"":""end_turn""},"
      ...
      begin
         if Method = "GET" and then Path = "/copilot_internal/v2/token" then
            ...
            Append
              (Res.Body_Data,
               ...
               & Long_Long_Image (Current_Unix_S + 7_200) & "}");
   ...
   begin
      ...
      Assert
        (Models_Calls = 1, "Provider should fetch the live Copilot catalogue");
      Assert
        (Message_Calls = 1,
   ...
   package LLM_GitHub_Copilot_Caller is new AUnit.Test_Caller
     (LLM_GitHub_Copilot_Tests.Test);
   ...
   begin
      Result.Add_Test
        (LLM_GitHub_Copilot_Caller.Create
        ...
        (LLM_GitHub_Copilot_Caller.Create
           ("LLM.GitHub_Copilot refreshes expired tokens before sending",
            LLM_GitHub_Copilot_Tests
              .Test_Copilot_Refreshes_Expired_Token_Then_Sends'
              Access));
